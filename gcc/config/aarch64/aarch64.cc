...
struct simd_immediate_info {
  ...
  simd_immediate_info(scalar_int_mode, unsigned HOST_WIDE_INT, insn_type = MOV,
                      modifier_type = LSL, unsigned int = 0);
...
inline simd_immediate_info ::simd_immediate_info(scalar_float_mode elt_mode_in,
                                                 rtx value_in)
...
/* Construct an integer immediate in which each element has mode ELT_MODE_IN
   ...
   fields.  */
inline simd_immediate_info ::simd_immediate_info(
    scalar_int_mode elt_mode_in, unsigned HOST_WIDE_INT value_in,
    insn_type insn_in, modifier_type modifier_in, unsigned int shift_in)
...
inline simd_immediate_info ::simd_immediate_info(scalar_mode elt_mode_in,
                                                 rtx base_in, rtx step_in)
...
inline simd_immediate_info ::simd_immediate_info(scalar_int_mode elt_mode_in,
...
static bool aarch64_vfp_is_call_or_return_candidate(machine_mode, const_tree,
                                                    machine_mode *, int *,
                                                    bool *, bool);
...
static const struct tune_params ampere1_tunings = {
    ...
    4,     /* issue_rate  */
    (AARCH64_FUSE_ADRP_ADD | AARCH64_FUSE_AES_AESMC | AARCH64_FUSE_MOV_MOVK |
...
static const aarch64_vec_issue_info neoversen2_vec_issue_info = {
    &neoversen2_scalar_issue_info, &neoversen2_advsimd_issue_info,
...
static const aarch64_vec_issue_info neoversev2_vec_issue_info = {
    &neoversev2_scalar_issue_info, &neoversev2_advsimd_issue_info,
...
#define AARCH64_ARCH(NAME, CORE, ARCH_IDENT, D, E)                             \
  {NAME,                                                                       \
   ...
   feature_deps::ARCH_IDENT().enable,                                          \
   NULL},
...
static tree handle_aarch64_vector_pcs_attribute(tree *node, tree name, tree,
  ...
  case ARM_PCS_SVE:
    error("the %qE attribute cannot be applied to an SVE function type", name);
    *no_add_attrs = true;
    return NULL_TREE;
...
{
  switch (pattern) {
#define CASE(UPPER, LOWER, VALUE)                                              \
  case AARCH64_SV_##UPPER:                                                     \
    return #LOWER;
...
pure_scalable_type_info::analyze_record (const_tree type)
{
  for (tree field = TYPE_FIELDS(type); field; field = TREE_CHAIN(field)) {
    ...
    poly_widest_int wide_bytepos =
        exact_div(wi::to_poly_widest(bitpos), BITS_PER_UNIT);
...
pure_scalable_type_info::add_piece (const piece &p)
{
  ...
  if (!pieces.is_empty()) {
    ...
    if (prev.orig_mode == p.orig_mode &&
        ...
        targetm.array_mode(p.orig_mode, nelems1 + nelems2).exists(&prev.mode)) {
...
aarch64_simd_abi (void)
{
  ...
  if (!simd_abi.initialized_p()) {
    HARD_REG_SET full_reg_clobbers = default_function_abi.full_reg_clobbers();
			...
			const char * branch_format)
{
  ...
  char label_buf[256];
  char buffer[128];
  ASM_GENERATE_INTERNAL_LABEL(label_buf, dest, CODE_LABEL_NUMBER(tmp_label));
...
{
  return (TARGET_SVE && (mode == VNx16BImode || mode == VNx8BImode ||
                         mode == VNx4BImode || mode == VNx2BImode));
}
...
aarch64_ldn_stn_vectors (machine_mode mode)
{
  ...
  if (vec_flags == (VEC_SVE_DATA | VEC_STRUCT))
    return exact_div(GET_MODE_SIZE(mode), BYTES_PER_SVE_VECTOR).to_constant();
  return 1;
}
				   ...
				   unsigned HOST_WIDE_INT nelems)
{
  ...
  if (aarch64_classify_vector_mode(struct_mode) == flags &&
      ...
      known_eq(GET_MODE_NUNITS(struct_mode), GET_MODE_NUNITS(mode) * nelems))
    return struct_mode;
...
aarch64_sve_data_mode (scalar_mode inner_mode, poly_uint64 nunits)
{
  ...
  if (inner_mode == GET_MODE_INNER(mode) &&
      known_eq(nunits, GET_MODE_NUNITS(mode)) && aarch64_sve_data_mode_p(mode))
    return mode;
  return opt_machine_mode();
}
...
{
  unsigned int vec_flags = aarch64_classify_vector_mode(mode);
  poly_uint64 vector_bits =
      (vec_flags & (VEC_PARTIAL | VEC_SVE_PRED) ? BITS_PER_SVE_VECTOR
...
{
  poly_uint64 vector_bits =
      (GET_MODE_CLASS(mode) == MODE_VECTOR_BOOL ? BITS_PER_SVE_VECTOR
                                                : GET_MODE_BITSIZE(mode));
  unsigned int elt_bits =
      vector_element_size(vector_bits, GET_MODE_NUNITS(mode));
				...
				poly_uint64 nunits)
{
  ...
  if ((vec_flags & VEC_SVE_DATA) &&
      multiple_p(BYTES_PER_SVE_VECTOR, GET_MODE_SIZE(element_mode),
                 &sve_nunits)) {
    ...
    if (maybe_ne(nunits, 0U)) {
      ...
      if (multiple_p(sve_nunits, nunits) &&
          aarch64_sve_data_mode(element_mode, nunits).exists(&sve_mode))
        return sve_mode;
    } else {
      ...
      if (aarch64_sve_data_mode(element_mode, sve_nunits).exists(&sve_mode))
        return sve_mode;
    }
  }

  /* Prefer to use 1 128-bit vector instead of 2 64-bit vectors.  */
  if (TARGET_SIMD && (vec_flags & VEC_ADVSIMD) && known_eq(nunits, 0U) &&
      known_eq(GET_MODE_BITSIZE(vector_mode), 64U) &&
      maybe_ge(GET_MODE_BITSIZE(element_mode) * GET_MODE_NUNITS(vector_mode),
               128U)) {
...
aarch64_hard_regno_nregs (unsigned regno, machine_mode mode)
{
  ...
  case FP_LO8_REGS: {
    unsigned int vec_flags = aarch64_classify_vector_mode(mode);
    if (vec_flags & VEC_SVE_DATA)
      return exact_div(GET_MODE_SIZE(mode), aarch64_vl_bytes(mode, vec_flags))
          .to_constant();
...
aarch64_takes_arguments_in_sve_regs_p (const_tree fntype)
{
  ...
  aarch64_init_cumulative_args(&args_so_far_v, NULL_TREE, NULL_RTX, NULL_TREE,
                               0, true);
...
aarch64_is_noplt_call_p (rtx sym)
{
  ...
  if (flag_pic && decl &&
      (!flag_plt || lookup_attribute("noplt", DECL_ATTRIBUTES(decl))) &&
...
aarch64_maxmin_plus_const (rtx_code code, rtx *operands, bool generate_p)
{
  ...
  if (!(diff == 0 || (diff == 1 && wi::gt_p(maxmin_val, sub_val, sgn)) ||
				   ...
				   enum aarch64_symbol_type type)
{
  ...
  case SYMBOL_TLSLE48: {
    ...
    case SYMBOL_TLSLE12:
      emit_insn(
          (mode == DImode ? gen_tlsle12_di : gen_tlsle12_si)(dest, tp, imm));
      break;
    case SYMBOL_TLSLE24:
      emit_insn(
          (mode == DImode ? gen_tlsle24_di : gen_tlsle24_si)(dest, tp, imm));
      break;
    case SYMBOL_TLSLE32:
      emit_insn((mode == DImode ? gen_tlsle32_di : gen_tlsle32_si)(dest, imm));
      emit_insn((mode == DImode ? gen_adddi3 : gen_addsi3)(dest, dest, tp));
      break;
    case SYMBOL_TLSLE48:
      emit_insn((mode == DImode ? gen_tlsle48_di : gen_tlsle48_si)(dest, imm));
      emit_insn((mode == DImode ? gen_adddi3 : gen_addsi3)(dest, dest, tp));
      break;
    default:
      gcc_unreachable();
...
{
  rtx tmp =
      expand_binop(GET_MODE(dest), binoptab, op0, op1, dest, 0, OPTAB_DIRECT);
  if (dest != tmp)
    emit_move_insn(dest, tmp);
}
...
aarch64_ptrue_all_mode (rtx x)
{
  ...
  if (!CONST_VECTOR_P(x) || !CONST_VECTOR_DUPLICATE_P(x) ||
...
aarch64_sve_same_pred_for_ptest_p (rtx *pred1, rtx *pred2)
{
  ...
  bool ptrue1_p =
      (pred1[0] == CONSTM1_RTX(mode) || INTVAL(pred1[1]) == SVE_KNOWN_PTRUE);
  bool ptrue2_p =
      (pred2[0] == CONSTM1_RTX(mode) || INTVAL(pred2[1]) == SVE_KNOWN_PTRUE);
...
{
  machine_mode src_mode = GET_MODE(src);
  return aarch64_sve_emit_int_cmp(target, mode, NE, src_mode, src,
                                  CONST0_RTX(src_mode));
}
...
aarch64_fold_sve_cnt_pat (aarch64_svpattern pattern, unsigned int nelts_per_vq)
{
  ...
  if (pattern >= AARCH64_SV_VL1 && pattern <= AARCH64_SV_VL8)
    ...
    switch (pattern) {
    case AARCH64_SV_POW2:
      return 1 << floor_log2(nelts);
    case AARCH64_SV_MUL4:
      return nelts & -4;
    case AARCH64_SV_MUL3:
      return (nelts / 3) * 3;
    case AARCH64_SV_ALL:
      return nelts;
    default:
...
aarch64_sve_cnt_immediate_p (poly_int64 value)
{
  ...
  return (value.coeffs[1] == factor && IN_RANGE(factor, 2, 16 * 16) &&
          (factor & 1) == 0 && factor <= 16 * (factor & -factor));
}
				  ...
				  unsigned int nelts_per_vq)
{
  ...
  if (pattern == AARCH64_SV_ALL && factor == 1)
    written =
        snprintf(buffer, sizeof(buffer), "%s%c\t%s", prefix, suffix, operands);
  else if (factor == 1)
    written = snprintf(buffer, sizeof(buffer), "%s%c\t%s, %s", prefix, suffix,
                       operands, svpattern_token(pattern));
  else
    written = snprintf(buffer, sizeof(buffer), "%s%c\t%s, %s, mul #%d", prefix,
                       suffix, operands, svpattern_token(pattern), factor);
  gcc_assert(written < sizeof(buffer));
  return buffer;
}
				      ...
				      const char *operands, rtx *cnt_pat)
{
  ...
  return aarch64_output_sve_cnt_immediate(prefix, operands, pattern, factor,
                                          nelts_per_vq);
}
					...
					unsigned int *nelts_per_vq_out)
{
  ...
  if (!const_vec_duplicate_p(x, &elt) || !poly_int_rtx_p(elt, &value))
    return false;
				...
				machine_mode mode)
{
  ...
  if ((val >> 32) == 0 || mode == SImode) {
    if (generate) {
      ...
      if (mode == SImode)
        emit_insn(
            gen_insv_immsi(dest, GEN_INT(16), GEN_INT((val >> 16) & 0xffff)));
      else
        emit_insn(
            gen_insv_immdi(dest, GEN_INT(16), GEN_INT((val >> 16) & 0xffff)));
    }
    return 2;
  }
  ...
  if (zero_match < 2 && one_match < 2) {
    ...
    if (i != 64) {
      if (generate) {
        ...
        emit_insn(
            gen_insv_immdi(dest, GEN_INT(i), GEN_INT((val >> i) & 0xffff)));
      }
      return 2;
    }
  }
        ...
        if (aarch64_check_bitmask(val, val2, (mask << i) | (mask << j))) {
          if (generate) {
            ...
            emit_insn(
                gen_insv_immdi(dest, GEN_INT(i), GEN_INT((val >> i) & 0xffff)));
            emit_insn(
                gen_insv_immdi(dest, GEN_INT(j), GEN_INT((val >> j) & 0xffff)));
          }
          return 3;
        }
  }
  ...
  for (i += 16; i < 64; i += 16) {
    if ((val2 & (mask << i)) == 0)
      continue;
    if (generate)
      emit_insn(gen_insv_immdi(dest, GEN_INT(i), GEN_INT((val >> i) & 0xffff)));
    num_insns++;
  }
...
aarch64_mov128_immediate (rtx imm)
{
  ...
  return aarch64_internal_mov_immediate(NULL_RTX, lo, false, DImode) +
             aarch64_internal_mov_immediate(NULL_RTX, hi, false, DImode) <=
         4;
}
		      ...
		      bool frame_related_p, bool emit_move_imm)
{
  ...
  if (moffset < 0x1000000 && ((!temp1 && !can_create_pseudo_p()) ||
  ...
  if (emit_move_imm) {
    ...
    temp1 = aarch64_force_temporary(mode, temp1, gen_int_mode(moffset, mode));
  }
  insn = emit_insn(offset < 0 ? gen_sub3_insn(dest, src, temp1)
		    ...
		    bool frame_related_p, bool emit_move_imm = true)
{
  gcc_assert(emit_move_imm || temp1 != NULL_RTX);
  gcc_assert(temp1 == NULL_RTX || !reg_overlap_mentioned_p(temp1, src));
  gcc_assert(temp1 == NULL_RTX || !frame_related_p ||
  ...
  poly_int64 poly_offset(factor, factor);
  if (src != const0_rtx && aarch64_sve_addvl_addpl_immediate_p(poly_offset)) {
    ...
    if (constant == 0 || frame_related_p) {
      ...
      if (frame_related_p) {
        ...
        add_reg_note(insn, REG_CFA_ADJUST_CFA,
                     gen_rtx_SET(dest, plus_constant(Pmode, src, poly_offset)));
      }
      src = dest;
      if (constant == 0)
        return;
  ...
  aarch64_add_offset_1(mode, dest, src, constant, temp1, frame_related_p,
                       emit_move_imm);
}
...
{
  aarch64_add_offset(mode, dest, src, rtx_to_poly_int64(offset_rtx), temp1,
                     temp2, false);
}
...
{
  aarch64_add_offset(Pmode, stack_pointer_rtx, stack_pointer_rtx, delta, temp1,
                     temp2, true, emit_move_imm);
}
...
{
  aarch64_add_offset(Pmode, stack_pointer_rtx, stack_pointer_rtx, -delta, temp1,
                     temp2, frame_related_p, emit_move_imm);
}
...
aarch64_expand_sve_dupq (rtx target, machine_mode mode, rtx src)
{
  ...
  insn_code icode =
      (BYTES_BIG_ENDIAN ? code_for_aarch64_vec_duplicate_vq_be(mode)
  ...
  if (BYTES_BIG_ENDIAN) {
    ...
    rtx sel =
        aarch64_gen_stepped_int_parallel(nelts_per_vq, nelts_per_vq - 1, -1);
...
aarch64_expand_sve_const_vector_sel (rtx target, rtx src)
{
  ...
  if (aarch64_svpattern_for_vl(pred_mode, npatterns) == AARCH64_NUM_SVPATTERNS)
    return NULL_RTX;
				 ...
				 unsigned int vl)
{
  ...
  emit_insn(gen_while(UNSPEC_WHILELO, DImode, mode, target, const0_rtx, limit));
  return target;
}
				 ...
				 bool allow_recurse_p)
{
  ...
  if (INTVAL(builder.elt(0)) == 0)
    if (rtx res = aarch64_expand_sve_const_pred_eor(target, builder, elt_size))
      return res;

  /* Try using TRN1 to permute two simpler constants.  */
  for (unsigned int i = elt_size; i <= 8; i *= 2)
    if (rtx res =
            aarch64_expand_sve_const_pred_trn(target, builder, elt_size, i))
      return res;
...
aarch64_expand_sve_const_pred (rtx target, rtx_vector_builder &builder)
{
  ...
  rtx_vector_builder int_builder(VNx16QImode, builder.npatterns(),
                                 builder.nelts_per_pattern());
  for (unsigned int i = 0; i < builder.encoded_nelts(); ++i)
    int_builder.quick_push(INTVAL(builder.elt(i)) ? constm1_rtx : const0_rtx);
...
aarch64_expand_mov_immediate (rtx dest, rtx imm)
{
  ...
  scalar_int_mode int_mode;
  if ((SYMBOL_REF_P(imm) || LABEL_REF_P(imm) || GET_CODE(imm) == CONST ||
    ...
    if (!offset.is_constant(&const_offset)) {
      ...
      if (base == const0_rtx && aarch64_sve_cnt_immediate_p(offset))
        ...
        if (base != const0_rtx) {
          ...
          aarch64_add_offset(int_mode, dest, base, offset, NULL_RTX, NULL_RTX,
                             false);
        } else
          aarch64_add_offset(int_mode, dest, base, offset, dest, NULL_RTX,
                             false);
      }
      return;
    }
    ...
    case SYMBOL_FORCE_TO_MEM:
      ...
      if (const_offset != 0 && targetm.cannot_force_const_mem(ptr_mode, imm)) {
        gcc_assert(can_create_pseudo_p());
        base = aarch64_force_temporary(int_mode, dest, base);
        aarch64_add_offset(int_mode, dest, base, const_offset, NULL_RTX,
                           NULL_RTX, false);
        return;
      }
  ...
  if (!CONST_INT_P(imm)) {
    ...
    if (GET_CODE(imm) == HIGH || aarch64_simd_valid_immediate(imm, NULL)) {
      emit_insn(gen_rtx_SET(dest, imm));
      return;
    }
...
aarch64_maybe_expand_sve_subreg_move (rtx dest, rtx src)
{
  ...
  if (!REG_P(dest) || !REG_P(src) ||
      ...
      (GET_MODE_UNIT_SIZE(GET_MODE(dest)) == GET_MODE_UNIT_SIZE(GET_MODE(src))))
    return false;
...
aarch64_split_sve_subreg_move (rtx dest, rtx ptrue, rtx src)
{
  ...
  emit_insn(gen_aarch64_pred(unspec, mode_with_wider_elts, dest, ptrue, src));
}
			     ...
			     const function_arg_info &arg)
{
  ...
  if (aarch64_vfp_is_call_or_return_candidate(arg.mode, arg.type, &dummymode,
                                              &nregs, NULL,
                                              !pcum || pcum->silent_p))
    return false;
...
aarch64_return_in_msb (const_tree valtype)
{
  ...
  if (!aarch64_composite_type_p(valtype, TYPE_MODE(valtype)) ||
      int_size_in_bytes(valtype) <= 0 || int_size_in_bytes(valtype) > 16)
    return false;
  ...
  /* But not a composite that is an HFA (Homogeneous Floating-point Aggregate)
     ...
     register(s).  */
  if (aarch64_vfp_is_call_or_return_candidate(
          TYPE_MODE(valtype), valtype, &dummy_mode, &dummy_int, NULL, false))
    return false;
			...
			bool outgoing ATTRIBUTE_UNUSED)
{
  ...
  if (aarch64_vfp_is_call_or_return_candidate(mode, type, &ag_mode, &count,
    ...
    if (sve_p) {
      ...
      gcc_assert(type && (aarch64_some_values_include_pst_objects_p(type) ||
...
aarch64_return_in_memory_1 (const_tree type)
{
  ...
  if (aarch64_vfp_is_call_or_return_candidate(TYPE_MODE(type), type, &ag_mode,
                                              &count, NULL, false))
    return false;
...
{
  CUMULATIVE_ARGS *pcum = get_cumulative_args(pcum_v);
  return aarch64_vfp_is_call_or_return_candidate(
      mode, type, &pcum->aapcs_vfp_rmode, nregs, NULL, pcum->silent_p);
}
    ...
    if (TREE_CODE(field) == FIELD_DECL) {
      ...
      if (DECL_BIT_FIELD_TYPE(field)) {
        ...
        if (!DECL_PACKED(field))
          bitfield_alignment = std::max(bitfield_alignment,
...
aarch64_layout_arg (cumulative_args_t pcum_v, const function_arg_info &arg)
{
  ...
  bool warn_pcs_change =
      (warn_psabi && !pcum->silent_p &&
       (currently_expanding_function_start || currently_expanding_gimple_stmt));
  ...
  if (type && pst_info.analyze_registers(type)) {
    ...
    gcc_assert(arg.named && pcum->pcs_variant == ARM_PCS_SVE &&
  ...
  allocate_nvrn = aarch64_vfp_is_call_candidate(pcum_v, mode, type, &nregs);
  ...
  if (allocate_nvrn) {
    ...
    if (nvrn + nregs <= NUM_FP_ARG_REGS) {
      ...
      if (!aarch64_composite_type_p(type, mode)) {
        ...
        for (i = 0; i < nregs; i++) {
          rtx tmp = gen_rtx_REG(pcum->aapcs_vfp_rmode, V0_REGNUM + nvrn + i);
          rtx offset =
              gen_int_mode(i * GET_MODE_SIZE(pcum->aapcs_vfp_rmode), Pmode);
          tmp = gen_rtx_EXPR_LIST(VOIDmode, tmp, offset);
          XVECEXP(par, 0, i) = tmp;
        }
        pcum->aapcs_reg = par;
  ...
  if (allocate_ncrn && (ncrn + nregs <= NUM_ARG_REGS)) {
    ...
    if (nregs == 0 || (nregs == 1 && !sve_p) ||
      ...
      for (i = 0; i < nregs; i++) {
        ...
        tmp = gen_rtx_EXPR_LIST(VOIDmode, tmp, GEN_INT(i * UNITS_PER_WORD));
        XVECEXP(par, 0, i) = tmp;
      }
      pcum->aapcs_reg = par;
    }
			      ...
			      bool silent_p)
{
  ...
  if (!silent_p && !TARGET_FLOAT && fntype && fntype != error_mark_node) {
    ...
    if (aarch64_vfp_is_call_or_return_candidate(TYPE_MODE(type), type, &mode,
                                                &nregs, NULL, false))
      aarch64_err_no_fpadvsimd(TYPE_MODE(type));
  }

  if (!silent_p && !TARGET_SVE && pcum->pcs_variant == ARM_PCS_SVE) {
    /* We can't gracefully recover at this point, so make this a
       fatal error.  */
    if (fndecl)
      fatal_error(input_location, "%qE requires the SVE ISA extension", fndecl);
    else
      fatal_error(input_location,
			      ...
			      const function_arg_info &arg)
{
  ...
  if (pcum->pcs_variant == ARM_PCS_AAPCS64 ||
      pcum->pcs_variant == ARM_PCS_SIMD || pcum->pcs_variant == ARM_PCS_SVE) {
    aarch64_layout_arg(pcum_v, arg);
    gcc_assert((pcum->aapcs_reg != NULL_RTX) != (pcum->aapcs_stack_words != 0));
    pcum->aapcs_arg_processed = false;
...
aarch64_function_arg_padding (machine_mode mode, const_tree type)
{
  ...
  if (type ? (INTEGRAL_TYPE_P(type) || SCALAR_FLOAT_TYPE_P(type) ||
              POINTER_TYPE_P(type))
...
aarch64_emit_probe_stack_range (HOST_WIDE_INT first, poly_int64 poly_size)
{
  ...
  if (size <= PROBE_INTERVAL) {
    ...
    emit_set_insn(reg1,
                  plus_constant(Pmode, stack_pointer_rtx, -(first + base)));
  ...
  /* The run-time loop is made up of 8 insns in the generic case while the
    ...
    emit_set_insn(reg1, plus_constant(Pmode, stack_pointer_rtx,
  ...
  /* Otherwise, do the same as above, but in a loop.  Note that we must be
    ...
    /* TEST_ADDR = SP + FIRST.  */
    emit_set_insn(reg1, plus_constant(Pmode, stack_pointer_rtx, -first));

    /* LAST_ADDR = SP + FIRST + ROUNDED_SIZE.  */
    HOST_WIDE_INT adjustment = -(first + rounded_size);
    if (!aarch64_uimm12_shift(adjustment)) {
      aarch64_internal_mov_immediate(reg2, GEN_INT(adjustment), true, Pmode);
...
#define SLOT_REQUIRED (-1)
  ...
  for (regno = R0_REGNUM; regno <= R30_REGNUM; regno++)
    if (df_regs_ever_live_p(regno) && !fixed_regs[regno] &&
        (regno == R30_REGNUM || !crtl->abi->clobbers_full_reg_p(regno)))
  ...
  if (frame_related_fp_reg_p && crtl->abi->id() == ARM_PCS_SVE &&
      BYTES_BIG_ENDIAN) {
  ...
  for (regno = V0_REGNUM; regno <= V31_REGNUM; regno++)
    if (known_eq(frame.reg_offset[regno], SLOT_REQUIRED)) {
      ...
      if (regno == last_fp_reg && has_align_gap &&
          known_eq(vector_save_size, 8) && multiple_p(offset, 16)) {
  ...
  poly_int64 above_outgoing_args =
      aligned_upper_bound(varargs_and_saved_regs_size + get_frame_size(),
  ...
  gcc_assert(
      multiple_p(crtl->outgoing_args_size, STACK_BOUNDARY / BITS_PER_UNIT));
  ...
  frame.is_scs_enabled =
      (!crtl->calls_eh_return && sanitize_flags_p(SANITIZE_SHADOW_CALL_STACK) &&
  ...
  if (frame.frame_size.is_constant(&const_size) &&
    ...
    frame.initial_adjust =
        (frame.hard_fp_offset + frame.below_hard_fp_saved_regs_size);
  ...
  gcc_assert(known_eq(frame.initial_adjust + frame.callee_adjust +
                          frame.sve_callee_adjust + frame.final_adjust,
                      frame.frame_size));
			  ...
			  HOST_WIDE_INT adjustment)
{
  switch (mode) {
  case E_DImode:
    return gen_storewb_pairdi_di(base, base, reg, reg2, GEN_INT(-adjustment),
                                 GEN_INT(UNITS_PER_WORD - adjustment));
  case E_DFmode:
    return gen_storewb_pairdf_di(base, base, reg, reg2, GEN_INT(-adjustment),
                                 GEN_INT(UNITS_PER_WORD - adjustment));
  case E_TFmode:
    return gen_storewb_pairtf_di(base, base, reg, reg2, GEN_INT(-adjustment),
...
aarch64_push_regs (unsigned regno1, unsigned regno2, HOST_WIDE_INT adjustment)
{
  ...
  insn = emit_insn(aarch64_gen_storewb_pair(mode, stack_pointer_rtx, reg1, reg2,
                                            adjustment));
  ...
  *cfi_ops = alloc_reg_note(REG_CFA_RESTORE, reg1, *cfi_ops);
  ...
  if (regno2 == INVALID_REGNUM) {
    ...
    *cfi_ops = alloc_reg_note(REG_CFA_RESTORE, reg2, *cfi_ops);
    emit_insn(aarch64_gen_loadwb_pair(mode, stack_pointer_rtx, reg1, reg2,
                                      adjustment));
  }
}
...
{
  rtx mem = gen_frame_mem(GET_MODE(reg), plus_constant(Pmode, base, offset));
			   ...
			   bool hard_fp_valid_p)
{
  ...
  for (regno = aarch64_next_callee_save(start, limit); regno <= limit;
       regno = aarch64_next_callee_save(regno + 1, limit)) {
    rtx reg, mem;
    poly_int64 offset;
    bool frame_related_p = aarch64_emit_cfi_for_reg_p(regno);

    if (skip_wb && (regno == cfun->machine->frame.wb_push_candidate1 ||
    ...
    if (mode == VNx2DImode && BYTES_BIG_ENDIAN)
      aarch64_adjust_sve_callee_save_base(mode, base_rtx, anchor_reg, offset,
                                          ptrue);
    else if (GP_REGNUM_P(regno) &&
             (!offset.is_constant(&const_offset) || const_offset >= 512)) {
      gcc_assert(known_eq(start_offset, 0));
      poly_int64 fp_offset = cfun->machine->frame.below_hard_fp_saved_regs_size;
      if (hard_fp_valid_p)
			      ...
			      unsigned limit, bool skip_wb, rtx *cfi_ops)
{
  ...
  for (regno = aarch64_next_callee_save(start, limit); regno <= limit;
    ...
    if (skip_wb && (regno == cfun->machine->frame.wb_pop_candidate1 ||
  ...
  for (unsigned regno = 0; regno <= LAST_SAVED_REGNUM; regno++)
    if (!fixed_regs[regno] && !crtl->abi->clobbers_full_reg_p(regno) &&
        (TEST_HARD_REG_BIT(extra_caller_saves, regno) ||
         bitmap_bit_p(in, regno) || bitmap_bit_p(gen, regno) ||
...
{
  rtx ptr_reg =
      gen_rtx_REG(Pmode, frame_pointer_needed ? HARD_FRAME_POINTER_REGNUM
  ...
  while (regno != last_regno) {
    ...
    if (aarch64_sve_mode_p(mode) || !satisfies_constraint_Ump(mem) ||
    ...
    rtx set2 = prologue_p ? gen_rtx_SET(mem2, reg2) : gen_rtx_SET(reg2, mem2);
					...
					bool final_adjustment_p)
{
  ...
  if (!poly_size.is_constant(&size)) {
    ...
    aarch64_add_offset(Pmode, temp1, CONST0_RTX(Pmode), poly_size, temp1, temp2,
                       false, true);
  ...
  if (rounded_size <= STACK_CLASH_MAX_UNROLL_PAGES * guard_size) {
    for (HOST_WIDE_INT i = 0; i < rounded_size; i += guard_size) {
      ...
      emit_stack_probe(
          plus_constant(Pmode, stack_pointer_rtx, guard_used_by_caller));
      emit_insn(gen_blockage());
    }
    dump_stack_clash_frame_info(PROBE_INLINE, size != rounded_size);
  } else {
    /* Compute the ending address.  */
    aarch64_add_offset(Pmode, temp1, stack_pointer_rtx, -rounded_size, temp1,
                       NULL, false, true);
    rtx_insn *insn = get_last_insn();
    ...
    insn = emit_insn(
        gen_probe_stack_range(stack_pointer_rtx, stack_pointer_rtx, temp1));
  ...
  if (residual) {
    ...
    if (residual >= min_probe_threshold) {
      if (dump_file)
        fprintf(
            dump_file,
            "Stack clash AArch64 prologue residuals: " HOST_WIDE_INT_PRINT_DEC
            " bytes, probing will be required."
            "\n",
            residual);

      emit_stack_probe(
          plus_constant(Pmode, stack_pointer_rtx, residual_probe_offset));
      emit_insn(gen_blockage());
    }
  }
}
...
aarch64_expand_prologue (void)
{
  ...
  poly_int64 chain_offset =
      (initial_adjust + callee_adjust - cfun->machine->frame.hard_fp_offset);
  ...
  if (emit_frame_chain) {
    if (callee_adjust == 0) {
      reg1 = R29_REGNUM;
      reg2 = R30_REGNUM;
      aarch64_save_callee_saves(saved_regs_offset, reg1, reg2, false, false);
    } else
      gcc_assert(known_eq(chain_offset, 0));
    aarch64_add_offset(Pmode, hard_frame_pointer_rtx, stack_pointer_rtx,
                       chain_offset, tmp1_rtx, tmp0_rtx, frame_pointer_needed);
    if (frame_pointer_needed && !frame_size.is_constant()) {
      ...
      if (!find_reg_note(insn, REG_CFA_ADJUST_CFA, NULL_RTX)) {
        rtx src = plus_constant(Pmode, stack_pointer_rtx, callee_offset);
        add_reg_note(insn, REG_CFA_ADJUST_CFA,
  ...
  if (maybe_ne(sve_callee_adjust, 0)) {
    gcc_assert(!flag_stack_clash_protection || known_eq(initial_adjust, 0));
    aarch64_allocate_and_probe_stack_space(
        tmp1_rtx, tmp0_rtx, sve_callee_adjust, !frame_pointer_needed, false);
    saved_regs_offset += sve_callee_adjust;
  }
  aarch64_save_callee_saves(saved_regs_offset, P0_REGNUM, P15_REGNUM, false,
                            emit_frame_chain);
...
aarch64_expand_epilogue (rtx_call_insn *sibcall)
{
  ...
  unsigned int last_gpr =
      (cfun->machine->frame.is_scs_enabled ? R29_REGNUM : R30_REGNUM);
  rtx cfi_ops = NULL;
  rtx_insn *insn;
  ...
  /* We can re-use the registers when:
     ...
     value.  */
  bool can_inherit_p =
      (initial_adjust.is_constant() && final_adjust.is_constant() &&
       (!flag_stack_clash_protection ||
        (known_lt(initial_adjust, guard_size - guard_used_by_caller) &&
         known_eq(sve_callee_adjust, 0))));

  /* We need to add memory barrier to prevent read from deallocated stack.  */
  bool need_barrier_p =
      maybe_ne(get_frame_size() + cfun->machine->frame.saved_varargs_size, 0);

  /* Emit a barrier to prevent loads from a deallocated stack.  */
  if (maybe_gt(final_adjust, crtl->outgoing_args_size) || cfun->calls_alloca ||
  ...
  rtx tmp0_rtx = gen_rtx_REG(Pmode, EP0_REGNUM);
  rtx tmp1_rtx = gen_rtx_REG(Pmode, EP1_REGNUM);
  if (frame_pointer_needed && (maybe_ne(final_adjust, 0) || cfun->calls_alloca))
    /* If writeback is used when restoring callee-saves, the CFA
       is restored on the instruction doing the writeback.  */
    aarch64_add_offset(Pmode, stack_pointer_rtx, hard_frame_pointer_rtx,
                       -callee_offset - below_hard_fp_saved_regs_size, tmp1_rtx,
                       tmp0_rtx, callee_adjust == 0);
  else
  ...
  aarch64_restore_callee_saves(callee_offset, P0_REGNUM, P15_REGNUM, false,
                               &cfi_ops);
  if (maybe_ne(sve_callee_adjust, 0))
  ...
  /* When shadow call stack is enabled, the scs_pop in the epilogue will
     ...
     way.  */
  aarch64_restore_callee_saves(callee_offset - sve_callee_adjust, R0_REGNUM,
                               last_gpr, callee_adjust != 0, &cfi_ops);
  ...
  aarch64_add_sp(
      tmp0_rtx, tmp1_rtx, initial_adjust,
      (!can_inherit_p || !crtl->is_leaf || df_regs_ever_live_p(EP0_REGNUM)));
			 ...
			 tree function)
{
  ...
  if (vcall_offset == 0)
    ...
    if (Pmode == ptr_mode)
      ...
      aarch64_emit_move(
          temp0, gen_rtx_ZERO_EXTEND(Pmode, gen_rtx_MEM(ptr_mode, addr)));

    if (vcall_offset >= -256 && vcall_offset < 4096 * POINTER_BYTES)
      addr = plus_constant(Pmode, temp0, vcall_offset);
    else {
      aarch64_internal_mov_immediate(temp1, GEN_INT(vcall_offset), true, Pmode);
    ...
    if (Pmode == ptr_mode)
      ...
      aarch64_emit_move(
          temp1, gen_rtx_SIGN_EXTEND(Pmode, gen_rtx_MEM(ptr_mode, addr)));
...
aarch64_case_values_threshold (void)
{
  ...
  if (optimize > 2 && aarch64_tune_params.max_case_values != 0)
...
aarch64_regno_ok_for_base_p (int regno, bool strict_p)
{
  ...
  return (GP_REGNUM_P(regno) || regno == SP_REGNUM ||
			...
			machine_mode mode, bool strict_p)
{
  ...
  /* (reg:P) */
  if ((REG_P(x) || SUBREG_P(x)) && GET_MODE(x) == Pmode) {
    type = ADDRESS_REG_REG;
    index = x;
    shift = 0;
  }
  /* (sign_extend:DI (reg:SI)) */
  else if ((GET_CODE(x) == SIGN_EXTEND || GET_CODE(x) == ZERO_EXTEND) &&
           GET_MODE(x) == DImode && GET_MODE(XEXP(x, 0)) == SImode) {
    type = (GET_CODE(x) == SIGN_EXTEND) ? ADDRESS_REG_SXTW : ADDRESS_REG_UXTW;
    index = XEXP(x, 0);
    shift = 0;
  }
  /* (mult:DI (sign_extend:DI (reg:SI)) (const_int scale)) */
           ...
           GET_MODE(XEXP(XEXP(x, 0), 0)) == SImode && CONST_INT_P(XEXP(x, 1))) {
    type = (GET_CODE(XEXP(x, 0)) == SIGN_EXTEND) ? ADDRESS_REG_SXTW
                                                 : ADDRESS_REG_UXTW;
  ...
  else if (GET_CODE(x) == AND && GET_MODE(x) == DImode &&
           GET_CODE(XEXP(x, 0)) == MULT &&
           GET_MODE(XEXP(XEXP(x, 0), 0)) == DImode &&
           CONST_INT_P(XEXP(XEXP(x, 0), 1)) && CONST_INT_P(XEXP(x, 1))) {
  ...
  else if (GET_CODE(x) == MULT && GET_MODE(x) == Pmode &&
           GET_MODE(XEXP(x, 0)) == Pmode && CONST_INT_P(XEXP(x, 1))) {
    type = ADDRESS_REG_REG;
    index = XEXP(x, 0);
    shift = exact_log2(INTVAL(XEXP(x, 1)));
  }
  /* (ashift:P (reg:P) (const_int shift)) */
  else if (GET_CODE(x) == ASHIFT && GET_MODE(x) == Pmode &&
           GET_MODE(XEXP(x, 0)) == Pmode && CONST_INT_P(XEXP(x, 1))) {
    type = ADDRESS_REG_REG;
    index = XEXP(x, 0);
    shift = INTVAL(XEXP(x, 1));
  ...
  if (!strict_p && SUBREG_P(index) &&
      contains_reg_of_mode[GENERAL_REGS][GET_MODE(SUBREG_REG(index))])
    index = SUBREG_REG(index);

  if (aarch64_sve_data_mode_p(mode)) {
    if (type != ADDRESS_REG_REG || (1 << shift) != GET_MODE_UNIT_SIZE(mode))
      return false;
  } else {
    if (shift != 0 &&
        !(IN_RANGE(shift, 1, 3) && known_eq(1 << shift, GET_MODE_SIZE(mode))))
      return false;
  }

  if (REG_P(index) && aarch64_regno_ok_for_index_p(REGNO(index), strict_p)) {
    info->type = type;
    info->offset = index;
    info->shift = shift;
    return true;
...
{
  return mode == SImode || mode == DImode || mode == SFmode || mode == DFmode ||
			  ...
			  aarch64_addr_query_type type)
{
  ...
  bool load_store_pair_p =
      (type == ADDR_QUERY_LDP_STP || type == ADDR_QUERY_LDP_STP_N ||
       mode == TImode || mode == TFmode || mode == TDmode ||
       ((!TARGET_SIMD || BYTES_BIG_ENDIAN) && advsimd_struct_p));
  ...
  bool allow_reg_index_p =
      (!load_store_pair_p &&
       ((vec_flags == 0 && known_lt(GET_MODE_SIZE(mode), 16)) ||
        vec_flags == VEC_ADVSIMD || vec_flags & VEC_SVE_DATA));
  ...
  /* On LE, for AdvSIMD, don't support anything other than POST_INC or
     REG addressing.  */
  if (advsimd_struct_p && TARGET_SIMD && !BYTES_BIG_ENDIAN &&
  ...
  case PLUS:
    ...
    if (!strict_p && REG_P(op0) && virt_or_elim_regno_p(REGNO(op0)) &&
    ...
    if (maybe_ne(GET_MODE_SIZE(mode), 0) &&
      ...
      if (aarch64_advsimd_partial_struct_mode_p(mode) &&
        ...
        return (aarch64_offset_7bit_signed_scaled_p(DImode, offset) &&
                (aarch64_offset_9bit_signed_unscaled_p(DImode, offset + 16) ||
                 offset_12bit_unsigned_scaled_p(DImode, offset + 16)));
      if (aarch64_advsimd_full_struct_mode_p(mode) &&
          known_eq(GET_MODE_SIZE(mode), 48))
        return (aarch64_offset_7bit_signed_scaled_p(TImode, offset) &&
                (aarch64_offset_9bit_signed_unscaled_p(TImode, offset + 32) ||
      ...
      if (aarch64_advsimd_partial_struct_mode_p(mode) &&
          known_eq(GET_MODE_SIZE(mode), 32))
        return (aarch64_offset_7bit_signed_scaled_p(DImode, offset) &&
                aarch64_offset_7bit_signed_scaled_p(DImode, offset + 16));
      if (aarch64_advsimd_full_struct_mode_p(mode) &&
      ...
      if (vec_flags == (VEC_SVE_DATA | VEC_STRUCT)) {
        poly_int64 end_offset =
            (offset + GET_MODE_SIZE(mode) - BYTES_PER_SVE_VECTOR);
        return (type == ADDR_QUERY_M
                    ? offset_4bit_signed_scaled_p(mode, offset)
                    : (offset_9bit_signed_scaled_p(SVE_BYTE_MODE, offset) &&
                       offset_9bit_signed_scaled_p(SVE_BYTE_MODE, end_offset)));
      }
  ...
  case LABEL_REF:
    ...
    if (!load_store_pair_p && GET_MODE_SIZE(mode).is_constant(&const_size) &&
        const_size >= 4) {
      poly_int64 offset;
      rtx sym = strip_offset_and_salt(x, &offset);
      return ((LABEL_REF_P(sym) ||
               (SYMBOL_REF_P(sym) && CONSTANT_POOL_ADDRESS_P(sym) &&
  ...
  case LO_SUM:
    ...
    if (allow_reg_index_p &&
      ...
      if (SYMBOL_REF_P(sym) && offset.is_constant(&const_offset) &&
...
aarch64_reinterpret_float_as_int (rtx value, unsigned HOST_WIDE_INT *intval)
{
  ...
  long res[2];
  real_to_target(res, CONST_DOUBLE_REAL_VALUE(value), REAL_MODE_FORMAT(mode));
...
aarch64_float_const_rtx_p (rtx x)
{
  ...
  if (CONST_DOUBLE_P(x) && SCALAR_FLOAT_MODE_P(mode) &&
...
aarch64_can_const_movi_rtx_p (rtx x, machine_mode mode)
{
  ...
  } else if (CONST_INT_P(x) && is_a<scalar_int_mode>(mode, &imode))
    ival = INTVAL(x);
  else
    return false;
...
aarch64_expand_call (rtx result, rtx mem, rtx callee_abi, bool sibcall)
{
  ...
  callee_abi =
      gen_rtx_UNSPEC(DImode, gen_rtvec(1, callee_abi), UNSPEC_CALLEE_ABI);
...
aarch64_select_cc_mode (RTX_CODE code, rtx x, rtx y)
{
  ...
  if (y == const0_rtx && (REG_P(x) || SUBREG_P(x)) &&
      (code == EQ || code == NE) && (mode_x == HImode || mode_x == QImode))
    return CC_Zmode;
  ...
  if ((mode_x == SImode || mode_x == DImode) && y == const0_rtx &&
  ...
  if ((mode_x == SImode || mode_x == DImode) && y == const0_rtx &&
      (code_x == AND) &&
      (code == EQ || code == NE || code == LT || code == GE || code == GT ||
       code == LE))
    return CC_NZVmode;
  ...
  if ((mode_x == SImode || mode_x == DImode) &&
      ...
      (code_x == ASHIFT || code_x == ASHIFTRT || code_x == LSHIFTRT ||
       code_x == ZERO_EXTEND || code_x == SIGN_EXTEND))
    return CC_SWPmode;

  /* Similarly for a negated operand, but we can only do this for
     equalities.  */
  if ((mode_x == SImode || mode_x == DImode) && (REG_P(y) || SUBREG_P(y)) &&
      (code == EQ || code == NE) && code_x == NEG)
    return CC_Zmode;

  /* A test for unsigned overflow from an addition.  */
  if ((mode_x == DImode || mode_x == TImode) && (code == LTU || code == GEU) &&
      code_x == PLUS && rtx_equal_p(XEXP(x, 0), y))
    return CC_Cmode;

  /* A test for unsigned overflow from an add with carry.  */
  if ((mode_x == DImode || mode_x == TImode) && (code == LTU || code == GEU) &&
      code_x == PLUS && CONST_SCALAR_INT_P(y) &&
      (rtx_mode_t(y, mode_x) ==
       (wi::shwi(1, mode_x) << (GET_MODE_BITSIZE(mode_x).to_constant() / 2))))
    return CC_ADCmode;

  /* A test for signed overflow.  */
  if ((mode_x == DImode || mode_x == TImode) && code == NE && code_x == PLUS &&
...
aarch64_get_condition_code_1 (machine_mode mode, enum rtx_code comp_code)
{
  ...
  case E_CC_NZCmode:
    switch (comp_code) {
    case NE:
      return AARCH64_NE; /* = any */
    case EQ:
      return AARCH64_EQ; /* = none */
    case GE:
      return AARCH64_PL; /* = nfrst */
    case LT:
      return AARCH64_MI; /* = first */
    case GEU:
      return AARCH64_CS; /* = nlast */
    case GTU:
      return AARCH64_HI; /* = pmore */
    case LEU:
      return AARCH64_LS; /* = plast */
    case LTU:
      return AARCH64_CC; /* = last */
    default:
      return -1;
    }
    break;
...
aarch64_const_vec_all_in_range_p (rtx vec,
				  ...
				  HOST_WIDE_INT maxval)
{
  if (!CONST_VECTOR_P(vec) || GET_MODE_CLASS(GET_MODE(vec)) != MODE_VECTOR_INT)
    return false;
  ...
  for (int i = 0; i < nunits; i++) {
    ...
    if (!CONST_INT_P(vec_elem) || !IN_RANGE(INTVAL(vec_elem), minval, maxval))
      return false;
  }
  return true;
}
...
aarch64_print_vector_float_operand (FILE *f, rtx x, bool negate)
{
  ...
  if (real_equal(&r, &dconst0))
    ...
    real_to_decimal_for_mode(float_buf, &r, buf_size, buf_size, 1,
                             GET_MODE(elt));
...
aarch64_print_operand (FILE *f, rtx x, int code)
{
  ...
  case 'q':
    if (!REG_P(x) || !FP_REGNUM_P(REGNO(x))) {
      output_operand_lossage(
          "incompatible floating point / vector register operand for '%%%c'",
          code);
      return;
    }
  ...
  case 'V':
    ...
    asm_fprintf(f, "%c%d", aarch64_sve_data_mode_p(GET_MODE(x)) ? 'z' : 'v',
  ...
  case 'R':
    if (REG_P(x) && FP_REGNUM_P(REGNO(x)) &&
      ...
      output_operand_lossage("incompatible register operand for '%%%c'", code);
    break;
    ...
    case REG:
      if (aarch64_sve_data_mode_p(GET_MODE(x))) {
        if (REG_NREGS(x) == 1)
          ...
          char suffix = sizetochar(GET_MODE_UNIT_BITSIZE(GET_MODE(x)));
          asm_fprintf(f, "{z%d.%c - z%d.%c}", REGNO(x) - V0_REGNUM, suffix,
...
#define buf_size 20
        char float_buf[buf_size] = {'\0'};
        real_to_decimal_for_mode(float_buf, CONST_DOUBLE_REAL_VALUE(x),
                                 buf_size, buf_size, 1, GET_MODE(x));
...
#undef buf_size
  ...
  case 'z': {
    ...
    if (!MEM_P(x) || (code == 'y' && maybe_ne(GET_MODE_SIZE(mode), 8) &&
				...
				aarch64_addr_query_type type)
{
  ...
  if (GET_MODE(x) != Pmode &&
      (!CONST_INT_P(x) || trunc_int_for_mode(INTVAL(x), Pmode) != INTVAL(x))) {
  ...
  if (aarch64_classify_address(&addr, x, mode, true, type))
    ...
    case ADDRESS_REG_IMM:
      ...
      if (vec_flags & VEC_ANY_SVE) {
        HOST_WIDE_INT vnum =
            exact_div(addr.const_offset, aarch64_vl_bytes(mode, vec_flags))
                .to_constant();
        asm_fprintf(f, "[%s, #%wd, mul vl]", reg_names[REGNO(addr.base)], vnum);
        return true;
      }
...
aarch64_regno_regclass (unsigned regno)
{
  ...
  if (FP_REGNUM_P(regno))
    return (FP_LO8_REGNUM_P(regno)  ? FP_LO8_REGS
            : FP_LO_REGNUM_P(regno) ? FP_LO_REGS
                                    : FP_REGS);
  ...
  if (GET_CODE(x) == PLUS && CONST_INT_P(XEXP(x, 1))) {
    ...
    if (GET_CODE(base) == PLUS) {
      ...
      if (virt_or_elim_regno_p(REGNO(op0))) {
        base = expand_binop(Pmode, add_optab, op0, offset_rtx, NULL_RTX, true,
                            OPTAB_DIRECT);
      ...
      base = expand_binop(Pmode, add_optab, op0, op1, NULL_RTX, true,
                          OPTAB_DIRECT);
    ...
    HOST_WIDE_INT size;
    if (GET_MODE_SIZE(mode).is_constant(&size)) {
      HOST_WIDE_INT base_offset = aarch64_anchor_offset(offset, size, mode);
      if (base_offset != 0) {
			  ...
			  secondary_reload_info *sri)
{
  ...
  if (reg_class_subset_p(rclass, FP_REGS) &&
      ...
      mode != VNx16QImode && (vec_flags & VEC_SVE_DATA) &&
  ...
  if (REG_P(x) &&
      ...
      mode == GET_MODE(x) && !TARGET_SIMD && FP_REGNUM_P(REGNO(x)) &&
  ...
  if (rclass == FP_REGS &&
      (mode == TImode || mode == TFmode || mode == TDmode) && CONSTANT_P(x))
    return GENERAL_REGS;
...
aarch64_secondary_memory_needed (machine_mode mode, reg_class_t class1,
				 reg_class_t class2)
{
  if (!TARGET_SIMD && reg_classes_intersect_p(class1, FP_REGS) &&
...
aarch64_trampoline_init (rtx m_tramp, tree fndecl, rtx chain_value)
{
  ...
  emit_block_move(gen_rtx_MEM(BLKmode, temp), assemble_trampoline_template(),
  ...
  maybe_emit_call_builtin___clear_cache(
      a_tramp, plus_constant(ptr_mode, a_tramp, TRAMPOLINE_SIZE));
}
...
aarch64_class_max_nregs (reg_class_t regclass, machine_mode mode)
{
  ...
  case FP_LO8_REGS:
    ...
    if ((vec_flags & VEC_SVE_DATA) &&
      ...
      return nregs;
    return (vec_flags & VEC_ADVSIMD ? CEIL(lowest_size, UNITS_PER_VREG)
...
aarch64_preferred_reload_class (rtx x, reg_class_t regclass)
{
  ...
  if (regclass == STACK_REG) {
    if (REG_P(x) && reg_class_subset_p(REGNO_REG_CLASS(REGNO(x)), POINTER_REGS))
      return regclass;
  ...
  if (!reg_class_subset_p(regclass, GENERAL_REGS) && GET_CODE(x) == PLUS) {
    ...
    gcc_assert(reg_class_subset_p(REGNO_REG_CLASS(REGNO(lhs)), POINTER_REGS));
    return NO_REGS;
  }
...
aarch64_output_casesi (rtx *operands)
{
  ...
  static const char *const patterns[4][2] = {
      {"ldrb\t%w3, [%0,%w1,uxtw]", "add\t%3, %4, %w3, sxtb #2"},
      {"ldrh\t%w3, [%0,%w1,uxtw #1]", "add\t%3, %4, %w3, sxth #2"},
      {"ldr\t%w3, [%0,%w1,uxtw #2]", "add\t%3, %4, %w3, sxtw #2"},
  ...
  snprintf(buf, sizeof(buf), "adr\t%%4, %s",
           targetm.strip_name_encoding(label));
...
aarch64_strip_shift (rtx x)
{
  ...
  if ((GET_CODE(op) == ASHIFT || GET_CODE(op) == ASHIFTRT ||
       GET_CODE(op) == LSHIFTRT || GET_CODE(op) == ROTATERT ||
       GET_CODE(op) == ROTATE) &&
      CONST_INT_P(XEXP(op, 1)))
    return XEXP(op, 0);

  if (GET_CODE(op) == MULT && CONST_INT_P(XEXP(op, 1)) &&
...
aarch64_strip_extend (rtx x, bool strip_shift)
{
  ...
  if (GET_CODE(op) == AND && GET_CODE(XEXP(op, 0)) == MULT &&
  ...
  /* Now handle extended register, as this may also have an optional
     left shift by 1..4.  */
  if (strip_shift && GET_CODE(op) == ASHIFT && CONST_INT_P(XEXP(op, 1)) &&
...
aarch64_strip_extend_vec_half (rtx x)
{
  if (GET_CODE(x) == ZERO_EXTEND || GET_CODE(x) == SIGN_EXTEND) {
    ...
    if (GET_CODE(x) == VEC_SELECT &&
        vec_series_highpart_p(GET_MODE(x), GET_MODE(XEXP(x, 0)), XEXP(x, 1)))
      x = XEXP(x, 0);
  }
  return x;
}
...
{
  if (GET_CODE(x) == VEC_DUPLICATE && is_a<scalar_mode>(GET_MODE(XEXP(x, 0)))) {
    x = XEXP(x, 0);
    if (GET_CODE(x) == VEC_SELECT)
      x = XEXP(x, 0);
...
aarch64_cheap_mult_shift_p (rtx x)
{
  ...
  if (GET_CODE(x) == ASHIFT && CONST_INT_P(op1) && UINTVAL(op1) <= 4)
    return true;
...
aarch64_rtx_mult_cost (rtx x, enum rtx_code code, int outer, bool speed)
{
  ...
  if (GET_MODE_CLASS(mode) == MODE_INT) {
    ...
    if (aarch64_shift_p(GET_CODE(x)) ||
        (CONST_INT_P(op1) && exact_log2(INTVAL(op1)) > 0)) {
      bool is_extend =
          GET_CODE(op0) == ZERO_EXTEND || GET_CODE(op0) == SIGN_EXTEND;
      if (speed) {
        if (compound_p) {
    ...
    if ((GET_CODE(op0) == ZERO_EXTEND && GET_CODE(op1) == ZERO_EXTEND) ||
        (GET_CODE(op0) == SIGN_EXTEND && GET_CODE(op1) == SIGN_EXTEND)) {
...
aarch64_branch_cost (bool speed_p, bool predictable_p)
{
  ...
  const struct cpu_branch_cost *branch_costs = aarch64_tune_params.branch_costs;
...
aarch64_if_then_else_costs (rtx op0, rtx op1, rtx op2, int *cost, bool speed)
{
  ...
  if (GET_CODE(op1) == PC || GET_CODE(op2) == PC) {
    ...
    if (GET_CODE(op1) == NEG || GET_CODE(op1) == NOT ||
...
aarch64_mask_and_shift_for_ubfiz_p (scalar_int_mode mode, rtx mask,
				    rtx shft_amnt)
{
  return CONST_INT_P(mask) && CONST_INT_P(shft_amnt) && INTVAL(mask) > 0 &&
         UINTVAL(shft_amnt) < GET_MODE_BITSIZE(mode) &&
         exact_log2((UINTVAL(mask) >> UINTVAL(shft_amnt)) + 1) >= 0 &&
         (UINTVAL(mask) & ((HOST_WIDE_INT_1U << UINTVAL(shft_amnt)) - 1)) == 0;
}
  ...
  *cost = COSTS_N_INSNS(1);
    ...
    *cost = 0;
    ...
    case MEM:
      if (speed) {
        ...
        *cost += COSTS_N_INSNS(aarch64_address_cost(address, mode, 0, speed));
      }
  ...
  case CONST_INT:
    ...
    if (x == const0_rtx)
      *cost = 0;
      ...
      machine_mode imode = known_le(GET_MODE_SIZE(mode), 4) ? SImode : DImode;
      *cost = COSTS_N_INSNS(
          aarch64_internal_mov_immediate(NULL_RTX, x, false, imode));
    }
    return true;
  ...
  cost_minus:
    ...
    *cost += rtx_cost(op0, mode, MINUS, 0, speed);
    ...
    if (is_a<scalar_int_mode>(mode) && aarch64_rtx_arith_op_extract_p(op1)) {
      if (speed)
        *cost += extra_cost->alu.extend_arith;

      op1 = aarch64_strip_extend(op1, true);
      *cost += rtx_cost(op1, VOIDmode, (enum rtx_code)GET_CODE(op1), 0, speed);
      return true;
    }
    ...
    if ((GET_CODE(new_op1) == MULT || aarch64_shift_p(GET_CODE(new_op1))) &&
        code != COMPARE) {
      *cost += aarch64_rtx_mult_cost(new_op1, MULT, (enum rtx_code)code, speed);
      return true;
    }
  ...
  cost_plus:
    ...
    *cost += rtx_cost(op1, mode, PLUS, 1, speed);
    ...
    if (is_a<scalar_int_mode>(mode) && aarch64_rtx_arith_op_extract_p(op0)) {
      if (speed)
        *cost += extra_cost->alu.extend_arith;

      op0 = aarch64_strip_extend(op0, true);
      *cost += rtx_cost(op0, VOIDmode, (enum rtx_code)GET_CODE(op0), 0, speed);
      return true;
    }
    ...
    if (GET_CODE(new_op0) == MULT || aarch64_shift_p(GET_CODE(new_op0))) {
      *cost += aarch64_rtx_mult_cost(new_op0, MULT, PLUS, speed);
      return true;
    }
  ...
  cost_logic:
    ...
    if (code == AND && GET_CODE(op0) == MULT && CONST_INT_P(XEXP(op0, 1)) &&
        CONST_INT_P(op1) &&
        aarch64_uxt_size(exact_log2(INTVAL(XEXP(op0, 1))), INTVAL(op1)) != 0) {
      /* This is a UBFM/SBFM.  */
    ...
    if (is_int_mode(mode, &int_mode)) {
      if (CONST_INT_P(op1)) {
        ...
        if (GET_CODE(op0) == ASHIFT &&
            aarch64_mask_and_shift_for_ubfiz_p(int_mode, op1, XEXP(op0, 1))) {
          *cost +=
              rtx_cost(XEXP(op0, 0), int_mode, (enum rtx_code)code, 0, speed);
          if (speed)
            *cost += extra_cost->alu.bfx;
        ...
        *cost += rtx_cost(new_op0, int_mode, (enum rtx_code)code, 0, speed);
        *cost += rtx_cost(op1, int_mode, (enum rtx_code)code, 1, speed);
      ...
      *cost += rtx_cost(op0, VOIDmode, ASHIFT, 0, speed);
      ...
      if (VECTOR_MODE_P(mode)) {
        ...
        if (GET_CODE(op1) == AND && REG_P(XEXP(op1, 0)) &&
            ...
            known_eq(INTVAL(XEXP(op1, 1)), GET_MODE_BITSIZE(mode) - 1)) {
  ...
  case MOD:
    ...
    if (CONST_INT_P(XEXP(x, 1)) && exact_log2(INTVAL(XEXP(x, 1))) > 0 &&
        (mode == SImode || mode == DImode)) {
      /* We expand to 4 instructions.  Reset the baseline.  */
      *cost = COSTS_N_INSNS(4);

      if (speed)
        *cost += 2 * extra_cost->alu.logical + 2 * extra_cost->alu.arith;
  ...
  case UMOD:
    if (speed) {
      ...
      if (VECTOR_MODE_P(mode))
        ...
        *cost +=
            (extra_cost->mult[mode == DImode].add +
             extra_cost->mult[mode == DImode].idiv + (code == MOD ? 1 : 0));
    }
    return false; /* All arguments need to be in registers.  */
  ...
  case IF_THEN_ELSE:
    return aarch64_if_then_else_costs(XEXP(x, 0), XEXP(x, 1), XEXP(x, 2), cost,
                                      speed);
  ...
  case UNSIGNED_FIX:
    ...
    if (GET_CODE(x) == UNSPEC) {
      ...
      if (uns_code == UNSPEC_FRINTA || uns_code == UNSPEC_FRINTM ||
          uns_code == UNSPEC_FRINTN || uns_code == UNSPEC_FRINTP ||
    ...
    if (GET_CODE(x) == MULT &&
      ...
      *cost += rtx_cost(XEXP(x, 0), VOIDmode, (rtx_code)code, 0, speed);
      return true;
    }
  ...
  case TRUNCATE:
    ...
    if (/* (truncate:DI  */
      ...
      *cost +=
          rtx_cost(XEXP(XEXP(XEXP(XEXP(x, 0), 0), 0), 0), mode, MULT, 0, speed);
      *cost +=
          rtx_cost(XEXP(XEXP(XEXP(XEXP(x, 0), 0), 1), 0), mode, MULT, 1, speed);
      return true;
    }
    break;
  case CONST_VECTOR: {
  ...
  if (dump_file && flag_aarch64_verbose_cost)
			    ...
			    reg_class_t from_i, reg_class_t to_i)
{
  ...
  if (to == TAILCALL_ADDR_REGS || to == POINTER_REGS || to == STUB_REGS)
    to = GENERAL_REGS;

  if (from == TAILCALL_ADDR_REGS || from == POINTER_REGS || from == STUB_REGS)
    from = GENERAL_REGS;
...
{
  return (!flag_trapping_math && flag_unsafe_math_optimizations &&
...
{
  if (ptrue)
    emit_insn(gen_aarch64_pred(UNSPEC_COND_FMUL, GET_MODE(dst), dst, ptrue,
                               src1, src2,
...
aarch64_emit_approx_sqrt (rtx dst, rtx src, bool recp)
{
  ...
  if (!recp) {
    if (!(flag_mlow_precision_sqrt ||
          (aarch64_tune_params.approx_modes->sqrt & AARCH64_APPROX_MODE(mode))))
      return false;

    if (!flag_finite_math_only || flag_trapping_math ||
        !flag_unsafe_math_optimizations || optimize_function_for_size_p(cfun))
      return false;
  } else
  ...
  machine_mode mmsk =
      (VECTOR_MODE_P(mode) ? related_int_vector_mode(mode).require()
  ...
  if (!recp) {
    ...
    if (pg) {
      ...
      emit_insn(gen_aarch64_pred_fcm(UNSPEC_COND_FCMNE, mode, xmsk, pg, hint,
                                     src, zero));
    } else {
      xmsk = gen_reg_rtx(mmsk);
      emit_insn(
          gen_rtx_SET(xmsk, gen_rtx_NEG(mmsk, gen_rtx_EQ(mmsk, src, zero))));
    }
  }
  ...
  if (!recp) {
    if (pg)
      /* Multiply nonzero source values by the corresponding intermediate
         ...
         otherwise.  */
      emit_insn(gen_cond(UNSPEC_COND_FMUL, mode, xdst, xmsk, xdst, src,
                         CONST0_RTX(mode)));
    else {
...
aarch64_emit_approx_div (rtx quo, rtx num, rtx den)
{
  ...
  bool use_approx_division_p =
      (flag_mlow_precision_div || (aarch64_tune_params.approx_modes->division &
                                   AARCH64_APPROX_MODE(mode)));
  ...
  if (flag_mlow_precision_div)
    iterations =
        (GET_MODE_INNER(mode) == DFmode ? aarch64_double_recp_precision
...
aarch64_vec_op_count::min_nonpred_cycles_per_iter () const
{
  ...
  cycles =
      std::max(cycles, {loads + stores, issue_info->loads_stores_per_cycle});
  cycles = std::max(cycles, {general_ops, issue_info->general_ops_per_cycle});
...
{
  return std::max(min_nonpred_cycles_per_iter(), min_pred_cycles_per_iter());
}
...
{
  dump_printf_loc(MSG_NOTE, vect_location, "  load operations = %d\n", loads);
  dump_printf_loc(MSG_NOTE, vect_location, "  store operations = %d\n", stores);
  dump_printf_loc(MSG_NOTE, vect_location, "  general operations = %d\n",
                  general_ops);
  if (sve_issue_info())
    dump_printf_loc(MSG_NOTE, vect_location, "  predicate operations = %d\n",
                    pred_ops);
  dump_printf_loc(MSG_NOTE, vect_location, "  reduction latency = %d\n",
                  reduction_latency);
...
public:
  ...
  unsigned int add_stmt_cost(
      int count, vect_cost_for_stmt kind, stmt_vec_info stmt_info, slp_tree,
      tree vectype, int misalign, vect_cost_model_location where) override;
		 ...
		 : aarch64_classify_vector_mode (vinfo->vector_mode))
{
  if (auto *issue_info = aarch64_tune_params.vec_costs->issue_info) {
    ...
    if (aarch64_tune_params.vec_costs == &neoverse512tvb_vector_cost) {
      ...
      m_ops.quick_push({&neoversev1_vec_issue_info, m_vec_flags, vf_factor});
    }
  }
}
...
record_potential_advsimd_unrolling (loop_vec_info loop_vinfo)
{
  ...
  /* Only handle cases in which the number of Advanced SIMD iterations
     ...
     would not.  */
  if (!LOOP_VINFO_NITERS_KNOWN_P(loop_vinfo) || aarch64_sve_vg.is_constant())
    return;
				    ...
				    int misalign ATTRIBUTE_UNUSED)
{
  ...
  case vector_stmt:
    return fp ? simd_costs->fp_stmt_cost : simd_costs->int_stmt_cost;
...
{
  if ((kind == vector_load || kind == unaligned_load || kind == vector_store ||
				   ...
				   unsigned int vec_flags)
{
  ...
  const simd_vec_cost *simd_costs = aarch64_simd_vec_costs_for_flags(vec_flags);
  if (is_float)
				    ...
				    fractional_cost stmt_cost)
{
  ...
  if (kind == vec_to_scalar && where == vect_epilogue &&
				 ...
				 aarch64_vec_op_count *ops)
{
  ...
  if (sve_issue && (kind == vector_stmt || kind == vec_to_scalar))
    if (tree type = vect_comparison_type(stmt_info)) {
      unsigned int base = (FLOAT_TYPE_P(type) ? sve_issue->fp_cmp_pred_ops
  ...
  if (sve_issue && (kind == scalar_load || kind == scalar_store) &&
      STMT_VINFO_MEMORY_ACCESS_TYPE(stmt_info) == VMAT_GATHER_SCATTER) {
				     ...
				     vect_cost_model_location where)
{
  ...
  bool in_inner_loop_p = (where == vect_body && stmt_info &&
  ...
  if (m_stp_sequence_cost != ~0U) {
    uint64_t cost = aarch64_stp_sequence_cost(count, kind, stmt_info, vectype);
  ...
  if (stmt_info && aarch64_use_new_vector_costs_p()) {
    ...
    stmt_cost =
        aarch64_detect_scalar_stmt_subtype(m_vinfo, kind, stmt_info, stmt_cost);

    if (vectype && m_vec_flags)
      stmt_cost = aarch64_detect_vector_stmt_subtype(m_vinfo, kind, stmt_info,
  ...
  if (stmt_info && vectype && aarch64_sve_mode_p(TYPE_MODE(vectype)))
    stmt_cost = aarch64_sve_adjust_stmt_cost(m_vinfo, kind, stmt_info, vectype,
                                             stmt_cost);

  if (stmt_info && aarch64_use_new_vector_costs_p()) {
    /* Account for any extra "embedded" costs that apply additively
       to the base cost calculated above.  */
    stmt_cost = aarch64_adjust_stmt_cost(kind, stmt_info, vectype, stmt_cost);
    ...
    if (loop_vinfo && (m_costing_for_scalar || where == vect_body) &&
  ...
  if (stmt_info && (kind == vector_store || kind == unaligned_store) &&
...
aarch64_vector_costs::determine_suggested_unroll_factor ()
{
  ...
  for (auto vec_ops : m_ops) {
    aarch64_simd_vec_issue_info const *vec_issue = vec_ops.simd_issue_info();
    if (!vec_issue)
      return 1;
    ...
    /* Check stores.  */
    if (vec_ops.stores > 0) {
      temp = CEIL(factor * vec_issue->stores_per_cycle, vec_ops.stores);
    ...
    if (vec_ops.general_ops > 0) {
      temp =
          CEIL(factor * vec_issue->general_ops_per_cycle, vec_ops.general_ops);
      unroll_factor = MIN(unroll_factor, temp);
    }
		  ...
		  unsigned int body_cost)
{
  ...
  if (dump_enabled_p())
    dump_printf_loc(MSG_NOTE, vect_location, "Original vector body cost = %d\n",
                    body_cost);
  ...
  if (vector_ops.sve_issue_info()) {
    ...
    if (aarch64_tune_params.vec_costs == &neoverse512tvb_vector_cost) {
      ...
      if (dump_enabled_p())
        dump_printf_loc(MSG_NOTE, vect_location, "Neoverse V1 estimate:\n");
    ...
    if (dump_enabled_p()) {
      dump_printf_loc(MSG_NOTE, vect_location, "Vector issue estimate:\n");
      vector_ops.dump();
    }
  }
  ...
  if (m_num_vector_iterations >= 1 && m_num_vector_iterations < threshold) {
    if (dump_enabled_p())
      dump_printf_loc(MSG_NOTE, vect_location,
...
aarch64_vector_costs::finish_cost (const vector_costs *uncast_scalar_costs)
{
  ...
  loop_vec_info loop_vinfo = dyn_cast<loop_vec_info>(m_vinfo);
  if (loop_vinfo && m_vec_flags && aarch64_use_new_vector_costs_p()) {
    m_costs[vect_body] =
        adjust_body_cost(loop_vinfo, scalar_costs, m_costs[vect_body]);
  ...
  if (!loop_vinfo && scalar_costs && m_stp_sequence_cost != ~0U) {
...
better_main_loop_than_p (const vector_costs *uncast_other) const
{
  ...
  if (bool(m_unrolled_advsimd_stmts) != bool(other->m_unrolled_advsimd_stmts)) {
  ...
  for (unsigned int i = 0; i < m_ops.length(); ++i) {
    if (dump_enabled_p()) {
      ...
      dump_printf_loc(MSG_NOTE, vect_location, "Issue info for %s loop:\n",
                      GET_MODE_NAME(this_loop_vinfo->vector_mode));
      this->m_ops[i].dump();
      dump_printf_loc(MSG_NOTE, vect_location, "Issue info for %s loop:\n",
                      GET_MODE_NAME(other_loop_vinfo->vector_mode));
    ...
    auto this_estimated_vf =
        (vect_vf_for_cost(this_loop_vinfo) * this->m_ops[i].vf_factor());
    auto other_estimated_vf =
        (vect_vf_for_cost(other_loop_vinfo) * other->m_ops[i].vf_factor());
    ...
    auto better_pred_limit_p = [](const aarch64_vec_op_count &a,
      ...
      if (a.pred_ops == 0 &&
          (b.min_pred_cycles_per_iter() > b.min_nonpred_cycles_per_iter())) {
        if (dump_enabled_p())
          dump_printf_loc(MSG_NOTE, vect_location,
		    ...
		    std::string *invalid_extension)
{
  ...
  for (arch = all_architectures; arch->name != NULL; arch++) {
    if (strlen(arch->name) == len && strncmp(arch->name, to_parse, len) == 0) {
				...
				const char *option_name)
{
  for (; flag->name != NULL; flag++) {
    if (length == strlen(flag->name) && !strncmp(flag->name, token, length))
      return flag->flag;
  }
			       ...
			       const char *option_name)
{
  ...
  while ((ntoken = strchr(specs, separator))) {
    ...
    unsigned token_ops =
        aarch64_parse_one_option_token(specs, token_length, flags, option_name);
...
{
  tune->fusible_ops = aarch64_parse_boolean_options(
      fuse_string, aarch64_fusible_pairs, tune->fusible_ops, "fuse=");
}
...
aarch64_parse_tune_string (const char *tune_string,
			    struct tune_params *tune)
{
  tune->extra_tuning_flags = aarch64_parse_boolean_options(
      tune_string, aarch64_tuning_flags, tune->extra_tuning_flags, "tune=");
}
...
aarch64_adjust_generic_arch_tuning (struct tune_params &current_tune)
{
  ...
  if (TARGET_SVE2)
    current_tune.extra_tuning_flags &= ~AARCH64_EXTRA_TUNE_CSE_SVE_VL_CONSTANTS;
}
...
aarch64_override_options_internal (struct gcc_options *opts)
{
  ...
  if ((flag_sanitize & SANITIZE_SHADOW_CALL_STACK) && !fixed_regs[R18_REGNUM])
  ...
  if (aarch64_autovec_preference == 1)
    SET_OPTION_IF_UNSET(opts, &global_options_set, aarch64_sve_compare_costs,
                        0);
  ...
  if (aarch64_tune_params.prefetch->l1_cache_line_size >= 0) {
    ...
    /* For a generic AArch64 target, cover the current range of cache line
       sizes.  */
    SET_OPTION_IF_UNSET(opts, &global_options_set,
                        param_destruct_interfere_size, 256);
    SET_OPTION_IF_UNSET(opts, &global_options_set,
  ...
  SET_OPTION_IF_UNSET(opts, &global_options_set, param_sched_pressure_algorithm,
  ...
  /* The maybe_set calls won't update the value if the user has explicitly set
     ...
     are equal.  */
  int probe_interval = param_stack_clash_protection_probe_interval;
  ...
  if (opts->x_flag_prefetch_loop_arrays < 0 && !opts->x_optimize_size &&
...
{
  enum aarch_parse_opt_result parse_res = aarch64_parse_tune(str, res);
...
aarch64_convert_sve_vector_bits (aarch64_sve_vector_bits_enum value)
{
  /* 128-bit SVE and Advanced SIMD modes use different register layouts
     ...
     case instead.  */
  if (value == SVE_SCALABLE || (value == SVE_128 && BYTES_BIG_ENDIAN))
  ...
  if (cpu && arch) {
    ...
    if (arch->arch != cpu->arch) {
      warning(0, "switch %<-mcpu=%s%> conflicts with %<-march=%s%> switch",
              aarch64_cpu_string, aarch64_arch_string);
    }
...
initialize_aarch64_code_model (struct gcc_options *opts)
{
  ...
  case AARCH64_CMODEL_SMALL:
    if (opts->x_flag_pic) {
      ...
      aarch64_cmodel = (flag_pic == 2 ? AARCH64_CMODEL_SMALL_PIC
...
{
  const struct processor *cpu = aarch64_get_tune_cpu(ptr->x_selected_tune);
  const struct processor *arch = aarch64_get_arch(ptr->x_selected_arch);
  std::string extension = aarch64_get_extension_string_for_isa_flags(
      ptr->x_aarch64_asm_isa_flags, arch->flags);
...
/* Handle the argument STR to the branch-protection= attribute.  */
 ...
 aarch64_handle_attr_branch_protection (const char* str)
 {
  ...
  enum aarch_parse_opt_result res =
      aarch_parse_branch_protection(str, &err_str);
  bool success = false;
  switch (res) {
  case AARCH_PARSE_MISSING_ARG:
...
aarch64_handle_attr_tune (const char *str)
{
  ...
  enum aarch_parse_opt_result parse_res = aarch64_parse_tune(str, &tmp_tune);
...
aarch64_process_one_target_attr (char *arg_str)
{
  ...
  for (p_attr = aarch64_attributes; p_attr->name; p_attr++) {
    ...
    if (invert && !p_attr->allow_neg) {
      error("pragma or attribute %<target(\"%s\")%> does not allow a negated "
            "form",
            str_to_check);
      return false;
    }
    ...
    case aarch64_attr_bool: {
      ...
      generate_option(p_attr->opt_num, NULL, !invert, CL_TARGET, &decoded);
      aarch64_handle_option(&global_options, &global_options_set, &decoded,
                            input_location);
      break;
    }
    ...
    case aarch64_attr_enum: {
      ...
      bool valid;
      int value;
      valid = opt_enum_arg_to_value(p_attr->opt_num, arg, &value, CL_TARGET);
      if (valid) {
        set_option(&global_options, NULL, p_attr->opt_num, value, NULL,
                   DK_UNSPECIFIED, input_location, global_dc);
      } else {
        error("pragma or attribute %<target(\"%s=%s\")%> is not valid",
              str_to_check, arg);
      }
      break;
    }
...
aarch64_process_target_attr (tree args)
{
  ...
  while (token) {
    ...
    if (!aarch64_process_one_target_attr(token)) {
      ...
      if (ext_res == AARCH_PARSE_OK)
        error("arch extension %<%s%> should be prefixed by %<+%>", token);
      else
  ...
  if (num_attrs != num_commas + 1) {
    error("malformed %<target(\"%s\")%> pragma or attribute",
...
aarch64_option_valid_attribute_p (tree fndecl, tree, tree args, int)
{
  ...
  old_optimize = build_optimization_node(&global_options, &global_options_set);
...
aarch64_can_inline_p (tree caller, tree callee)
{
  ...
  struct cl_target_option *callee_opts = TREE_TARGET_OPTION(
      callee_tree ? callee_tree : target_option_default_node);
  ...
  if (caller_opts->x_aarch64_cmodel_var != callee_opts->x_aarch64_cmodel_var)
    return false;

  if (caller_opts->x_aarch64_tls_dialect != callee_opts->x_aarch64_tls_dialect)
    return false;
  ...
  if (!aarch64_tribools_ok_for_inlining_p(
          ...
          callee_opts->x_aarch64_fix_a53_err835769, 2,
          TARGET_FIX_ERR_A53_835769_DEFAULT))
    return false;

  if (!aarch64_tribools_ok_for_inlining_p(
          caller_opts->x_aarch64_fix_a53_err843419,
          callee_opts->x_aarch64_fix_a53_err843419, 2,
  ...
  if (!aarch64_tribools_ok_for_inlining_p(
          ...
          callee_opts->x_flag_omit_leaf_frame_pointer, 2, 1))
    return false;
...
aarch64_classify_symbol (rtx x, HOST_WIDE_INT offset)
{
  ...
  if (SYMBOL_REF_P(x)) {
    ...
    case AARCH64_CMODEL_TINY:
      ...
      if ((flag_pic || SYMBOL_REF_WEAK(x)) && !aarch64_symbol_binds_local_p(x))
...
{
  if (!target || GET_MODE(target) != Pmode || !register_operand(target, Pmode))
...
aarch64_build_builtin_va_list (void)
{
  ...
  va_list_name = build_decl(BUILTINS_LOCATION, TYPE_DECL,
                            get_identifier("__va_list"), va_list_type);
  DECL_ARTIFICIAL(va_list_name) = 1;
  TYPE_NAME(va_list_type) = va_list_name;
  TYPE_STUB_DECL(va_list_type) = va_list_name;

  /* Create the fields.  */
  f_stack = build_decl(BUILTINS_LOCATION, FIELD_DECL, get_identifier("__stack"),
                       ptr_type_node);
  f_grtop = build_decl(BUILTINS_LOCATION, FIELD_DECL,
                       get_identifier("__gr_top"), ptr_type_node);
  f_vrtop = build_decl(BUILTINS_LOCATION, FIELD_DECL,
                       get_identifier("__vr_top"), ptr_type_node);
  f_groff = build_decl(BUILTINS_LOCATION, FIELD_DECL,
                       get_identifier("__gr_offs"), integer_type_node);
  f_vroff = build_decl(BUILTINS_LOCATION, FIELD_DECL,
                       get_identifier("__vr_offs"), integer_type_node);
...
aarch64_expand_builtin_va_start (tree valist, rtx nextarg ATTRIBUTE_UNUSED)
{
  ...
  if (cfun->va_list_fpr_size)
    vr_save_area_size =
        MIN((NUM_FP_ARG_REGS - cum->aapcs_nvrn) * UNITS_PER_VREG,
  ...
  stack = build3(COMPONENT_REF, TREE_TYPE(f_stack), valist, f_stack, NULL_TREE);
  grtop = build3(COMPONENT_REF, TREE_TYPE(f_grtop), valist, f_grtop, NULL_TREE);
  vrtop = build3(COMPONENT_REF, TREE_TYPE(f_vrtop), valist, f_vrtop, NULL_TREE);
  groff = build3(COMPONENT_REF, TREE_TYPE(f_groff), valist, f_groff, NULL_TREE);
  vroff = build3(COMPONENT_REF, TREE_TYPE(f_vroff), valist, f_vroff, NULL_TREE);
  ...
  vr_offset = ROUND_UP(gr_save_area_size, STACK_BOUNDARY / BITS_PER_UNIT);
			      ...
			      gimple_seq *post_p ATTRIBUTE_UNUSED)
{
  ...
  unsigned int abi_break;
  unsigned int abi_break_packed;
  align = aarch64_function_arg_alignment(mode, type, &abi_break,
                                         &abi_break_packed) /
          BITS_PER_UNIT;
  ...
  if (aarch64_vfp_is_call_or_return_candidate(mode, type, &ag_mode, &nregs,
    ...
    f_top = build3(COMPONENT_REF, TREE_TYPE(f_vrtop), unshare_expr(valist),
                   f_vrtop, NULL_TREE);
    f_off = build3(COMPONENT_REF, TREE_TYPE(f_vroff), unshare_expr(valist),
                   f_vroff, NULL_TREE);
    ...
    f_top = build3(COMPONENT_REF, TREE_TYPE(f_grtop), unshare_expr(valist),
                   f_grtop, NULL_TREE);
    f_off = build3(COMPONENT_REF, TREE_TYPE(f_groff), unshare_expr(valist),
                   f_groff, NULL_TREE);
  ...
  t = build2(GE_EXPR, boolean_type_node, off, build_int_cst(TREE_TYPE(off), 0));
  ...
  if (align > 8) {
    ...
    t = build2(BIT_AND_EXPR, TREE_TYPE(t), t, build_int_cst(TREE_TYPE(t), -16));
    roundup = build2(MODIFY_EXPR, TREE_TYPE(arg), arg, t);
  } else
    roundup = NULL;
  /* Advance ap.__stack  */
  t = fold_build_pointer_plus_hwi(arg, size + 7);
  t = build2(BIT_AND_EXPR, TREE_TYPE(t), t, build_int_cst(TREE_TYPE(t), -8));
  ...
  if (is_ha) {
    ...
    t = build2(MODIFY_EXPR, field_t, build1(INDIRECT_REF, field_t, tmp_ha),
    ...
    for (i = 1; i < nregs; ++i) {
      ...
      u = build2(
          ...
          build2(MEM_REF, field_t, tmp_ha,
                 build_int_cst(field_ptr_t, (i * int_size_in_bytes(field_t)))),
				...
				int *pretend_size ATTRIBUTE_UNUSED, int no_rtl)
{
  ...
  if (!no_rtl) {
    ...
    if (vr_saved > 0) {
      ...
      off =
          -ROUND_UP(gr_saved * UNITS_PER_WORD, STACK_BOUNDARY / BITS_PER_UNIT);
...
aarch64_member_type_forces_blk (const_tree field_or_array, machine_mode mode)
{
  ...
  if (aarch64_sve::builtin_type_p(type, &num_zr, &num_pr) && num_pr != 0) {
    if (TREE_CODE(field_or_array) == ARRAY_TYPE)
      return !simple_cst_equal(TYPE_SIZE(field_or_array), TYPE_SIZE(type));
			 ...
			 unsigned int *warn_psabi_flags)
{
  ...
  case REAL_TYPE:
    mode = TYPE_MODE(type);
    if (mode != DFmode && mode != SFmode && mode != TFmode && mode != HFmode &&
  ...
  case ARRAY_TYPE: {
    ...
    count = aapcs_vfp_sub_candidate(TREE_TYPE(type), modep, warn_psabi_flags);
    if (count == -1 || !index || !TYPE_MAX_VALUE(index) ||
        !tree_fits_uhwi_p(TYPE_MAX_VALUE(index)) || !TYPE_MIN_VALUE(index) ||
        !tree_fits_uhwi_p(TYPE_MIN_VALUE(index)) || count < 0)
      return -1;
  ...
  case RECORD_TYPE: {
    ...
    for (field = TYPE_FIELDS(type); field; field = TREE_CHAIN(field)) {
      ...
      else if (DECL_BIT_FIELD(field) && integer_zerop(DECL_SIZE(field))) {
      ...
      sub_count =
          aapcs_vfp_sub_candidate(TREE_TYPE(field), modep, warn_psabi_flags);
      if (sub_count < 0)
        return -1;
      count += sub_count;
    }
			  ...
			  machine_mode mode)
{
  ...
  if (mode == BLKmode || GET_MODE_CLASS(mode) == MODE_COMPLEX_FLOAT ||
					 ...
					 bool silent_p)
{
  ...
  if ((!composite_p && (GET_MODE_CLASS(mode) == MODE_FLOAT ||
    ...
    *count = 2;
    ...
    int ag_count = aapcs_vfp_sub_candidate(type, &new_mode, &warn_psabi_flags);
    if (ag_count > 0 && ag_count <= HA_MAX_NUM_FLDS) {
      static unsigned last_reported_type_uid;
      unsigned uid = TYPE_UID(TYPE_MAIN_VARIANT(type));
      int alt;
      if (!silent_p && warn_psabi && warn_psabi_flags &&
          uid != last_reported_type_uid &&
          ((alt = aapcs_vfp_sub_candidate(type, &new_mode, NULL)) !=
           ag_count)) {
        const char *url10 = CHANGES_ROOT_URL "gcc-10/changes.html#empty_base";
        const char *url12 =
...
{
  if (TARGET_SVE && maybe_ne(width, 128) &&
...
aarch64_cmp_autovec_modes (machine_mode sve_m, machine_mode asimd_m)
{
  ...
  if (aarch64_tune_params.sve_width == SVE_SCALABLE && !prefer_asimd &&
      !prefer_sve)
...
aarch64_autovectorize_vector_modes (vector_modes *modes, bool)
{
  ...
  unsigned int sve_i =
      (TARGET_SVE && !only_asimd_p) ? 0 : ARRAY_SIZE(sve_modes);
  unsigned int advsimd_i = 0;

  while (!only_sve_p && advsimd_i < ARRAY_SIZE(advsimd_modes)) {
    if (sve_i < ARRAY_SIZE(sve_modes) &&
        aarch64_cmp_autovec_modes(sve_modes[sve_i], advsimd_modes[advsimd_i]))
...
is_madd_op (enum attr_type t1)
{
  ...
  enum attr_type mlatypes[] = {
      TYPE_MLA,    TYPE_MLAS,   TYPE_SMLAD,   TYPE_SMLADX, TYPE_SMLAL,
      TYPE_SMLALD, TYPE_SMLALS, TYPE_SMLALXY, TYPE_SMLAWX, TYPE_SMLAWY,
      TYPE_SMLAXY, TYPE_SMMLA,  TYPE_UMLAL,   TYPE_UMLALS, TYPE_SMLSD,
      TYPE_SMLSDX, TYPE_SMLSLD};
...
dep_between_memop_and_curr (rtx memop)
{
  ...
  for (opno = 1; opno < recog_data.n_operands; opno++) {
    ...
    if (REG_P(operand) && reg_overlap_mentioned_p(load_reg, operand))
      return true;
  }
  return false;
}
...
{
  return (CONST_INT_P(base_or_step) && IN_RANGE(INTVAL(base_or_step), -16, 15));
}
...
aarch64_sve_bitmask_immediate_p (rtx x)
{
  ...
  return (const_vec_duplicate_p(x, &elt) && CONST_INT_P(elt) &&
          aarch64_bitmask_imm(INTVAL(elt), GET_MODE_INNER(GET_MODE(x))));
}
...
{
  x = unwrap_const_vec_duplicate(x);
  return (CONST_INT_P(x) && (signed_p ? IN_RANGE(INTVAL(x), -16, 15)
...
aarch64_sve_float_arith_immediate_p (rtx x, bool negate_p)
{
  ...
  if (!const_vec_duplicate_p(x, &elt) || !CONST_DOUBLE_P(elt))
    return false;
...
aarch64_sve_float_mul_immediate_p (rtx x)
{
  ...
  return (const_vec_duplicate_p(x, &elt) && CONST_DOUBLE_P(elt) &&
				 ...
				 enum simd_immediate_check which)
{
  ...
  if (val32 == (val64 >> 32)) {
    ...
    if (which == AARCH64_CHECK_MOV && val16 == (val32 >> 16) &&
        val8 == (val16 >> 8)) {
      if (info)
...
aarch64_sve_ptrue_svpattern_p (rtx x, struct simd_immediate_info *info)
{
  ...
  if (info) {
    aarch64_svpattern pattern = (aarch64_svpattern)INTVAL(XVECEXP(x, 0, 0));
			      ...
			      enum simd_immediate_check which)
{
  ...
  rtx base, step;
  unsigned int n_elts;
  if (CONST_VECTOR_P(op) && CONST_VECTOR_DUPLICATE_P(op))
    n_elts = CONST_VECTOR_NPATTERNS(op);
  else if ((vec_flags & VEC_SVE_DATA) && const_vec_series_p(op, &base, &step)) {
    ...
    return true;
  } else if (CONST_VECTOR_P(op) && CONST_VECTOR_NUNITS(op).is_constant(&n_elts))
    /* N_ELTS set above.  */;
  else
    return false;

  scalar_float_mode elt_float_mode;
  if (n_elts == 1 && is_a<scalar_float_mode>(elt_mode, &elt_float_mode)) {
  ...
  for (unsigned int i = 0; i < 8; i++)
    val64 |= ((unsigned HOST_WIDE_INT)bytes[i % nbytes] << (i * BITS_PER_UNIT));
...
{
  rtx base, step;
  if (const_vec_series_p(x, &base, &step) && base == const0_rtx &&
      aarch64_sve_index_immediate_p(step))
    return step;
  return NULL_RTX;
}
...
aarch64_mov_operand_p (rtx x, machine_mode mode)
{
  ...
  return aarch64_classify_symbolic_expression(x) == SYMBOL_TINY_ABSOLUTE;
}
				       ...
				       bool high)
{
  ...
  for (i = 0; i < count_ideal; i++) {
    ...
    if (!CONST_INT_P(elt_op) || INTVAL(elt_ideal) != INTVAL(elt_op))
      return false;
  }
  return true;
}
			  ...
			  const_tree exp)
{
  ...
  if (lane < low || lane >= high) {
    if (exp)
      error_at(EXPR_LOCATION(exp), "lane %wd out of range %wd - %wd", lane, low,
               high - 1);
    else
...
{
  return MEM_P(op) && (GET_CODE(XEXP(op, 0)) == POST_INC || REG_P(XEXP(op, 0)));
}
...
aarch64_sve_ld1r_operand_p (rtx op)
{
  ...
  return (MEM_P(op) && is_a<scalar_mode>(GET_MODE(op), &mode) &&
...
aarch64_sve_ldnf1_operand_p (rtx op)
{
  ...
  return (MEM_P(op) &&
          aarch64_classify_address(&addr, XEXP(op, 0), GET_MODE(op), false) &&
          addr.type == ADDRESS_REG_IMM);
}
				...
				unsigned int count)
{
  ...
  if (!reg_overlap_mentioned_p(operands[0], operands[1]) || rdest < rsrc)
    for (i = 0; i < count; i++)
      emit_move_insn(gen_rtx_REG(mode, rdest + i), gen_rtx_REG(mode, rsrc + i));
  else
    for (i = 0; i < count; i++)
...
aarch64_simd_make_constant (rtx vals)
{
  ...
  if (const_vec != NULL_RTX && aarch64_simd_valid_immediate(const_vec, NULL))
    /* Load using MOVI/MVNI.  */
    return const_vec;
...
aarch64_expand_vector_init (rtx target, rtx vals)
{
  ...
  if (XVECLEN(vals, 0) == 2 && VECTOR_MODE_P(GET_MODE(XVECEXP(vals, 0, 0)))) {
    ...
    gcc_assert(GET_MODE_INNER(narrow_mode) == inner_mode &&
               known_eq(GET_MODE_SIZE(mode), 2 * GET_MODE_SIZE(narrow_mode)));
    emit_insn(gen_aarch64_vec_concat(narrow_mode, target, XVECEXP(vals, 0, 0),
                                     XVECEXP(vals, 0, 1)));
    return;
  }
  ...
  if (n_var == n_elts && n_elts <= 16) {
    ...
    if (maxv == 1) {
      ...
      if (n_elts == 2 && (inner_mode == E_DImode || inner_mode == E_DFmode))

      {
        ...
        if (memory_operand(x0, inner_mode) && memory_operand(x1, inner_mode) &&
 ...
 (rtx target, const rtx_vector_builder &builder, int nelts, int nelts_reqd)
{
  ...
  for (int i = nelts_reqd - 1;
       i >= 0 && valid_for_const_vector_p(elem_mode, builder.elt(i)); i--)
				...
				int nelts, int nelts_reqd)
{
  ...
  if (aarch64_sve_expand_vector_init_handle_trailing_same_elem(target, builder,
                                                               nelts_reqd))
    return true;
  ...
  bool did_even_p =
      aarch64_sve_expand_vector_init(tmp1, v_even, nelts, nelts_reqd / 2);

  rtx tmp2 = gen_reg_rtx(mode);
  bool did_odd_p =
      aarch64_sve_expand_vector_init(tmp2, v_odd, nelts, nelts_reqd / 2);
...
aarch64_sve_expand_vector_init (rtx target, rtx vals)
{
  ...
  if (nelts < 4 || !aarch64_sve_expand_vector_init(target, v, nelts, nelts))
...
aarch64_prepare_sve_int_fma (rtx *operands, rtx_code code)
{
  ...
  if (rtx shifts = aarch64_convert_mult_to_shift(operands[2], code)) {
    rtx product = expand_binop(mode, vashl_optab, operands[1], shifts, NULL_RTX,
                               true, OPTAB_DIRECT);
    force_expand_binop(mode, code == PLUS ? add_optab : sub_optab, operands[3],
                       product, operands[0], true, OPTAB_DIRECT);
    return true;
  }
...
aarch64_prepare_sve_cond_int_fma (rtx *operands, rtx_code code)
{
  ...
  if (rtx shifts = aarch64_convert_mult_to_shift(operands[3], code)) {
    rtx product = expand_binop(mode, vashl_optab, operands[2], shifts, NULL_RTX,
                               true, OPTAB_DIRECT);
    emit_insn(gen_cond(code, mode, operands[0], operands[1], operands[4],
                       product, operands[5]));
    return true;
  }
				...
				tree fndecl)
{
  ...
  std::string extension =
      aarch64_get_extension_string_for_isa_flags(isa_flags, this_arch->flags);
					...
					bool record_p)
{
  ...
  rtx pa = gen_patchable_area(GEN_INT(patch_area_size), GEN_INT(record_p));
  ...
  if (!insn || !INSN_P(insn) || GET_CODE(PATTERN(insn)) != UNSPEC_VOLATILE ||
...
aarch64_start_file (void)
{
  ...
  std::string extension = aarch64_get_extension_string_for_isa_flags(
      default_isa_flags, default_arch->flags);
			     ...
			     rtx mem, rtx model_rtx)
{
  if (mode == TImode)
    emit_insn(gen_aarch64_load_exclusive_pair(
        gen_lowpart(DImode, rval), gen_highpart(DImode, rval), mem, model_rtx));
  else
...
#define DEF0(B, N)                                                             \
  {                                                                            \
    "__aarch64_" #B #N "_relax", "__aarch64_" #B #N "_acq",                    \
        "__aarch64_" #B #N "_rel", "__aarch64_" #B #N "_acq_rel",              \
        "__aarch64_" #B #N "_sync"                                             \
  }

#define DEF4(B)                                                                \
  DEF0(B, 1), DEF0(B, 2), DEF0(B, 4), DEF0(B, 8), { NULL, NULL, NULL, NULL }
...
aarch64_expand_compare_and_swap (rtx operands[])
{
  ...
  if (TARGET_LSE) {
    ...
    emit_insn(gen_aarch64_compare_and_swap_lse(mode, rval, mem, newval, mod_s));
    ...
    rval = emit_library_call_value(func, NULL_RTX, LCT_NORMAL, r_mode, oldval,
                                   mode, newval, mode, XEXP(mem, 0), Pmode);
    cc_reg = aarch64_gen_compare_reg_maybe_ze(NE, rval, oldval, mode);
  } else {
    ...
    emit_insn(GEN_FCN(code)(rval, mem, oldval, newval, is_weak, mod_s, mod_f));
...
aarch64_emit_post_barrier (enum memmodel model)
{
  ...
  if (is_mm_sync(model) &&
      (base_model == MEMMODEL_ACQUIRE || base_model == MEMMODEL_ACQ_REL ||
...
aarch64_split_compare_and_swap (rtx operands[])
{
  ...
  x = gen_rtx_IF_THEN_ELSE(VOIDmode, x, gen_rtx_LABEL_REF(Pmode, label2),
                           pc_rtx);
  ...
  if (!is_weak) {
    ...
    x = gen_rtx_IF_THEN_ELSE(VOIDmode, x, gen_rtx_LABEL_REF(Pmode, label1),
                             pc_rtx);
    aarch64_emit_unlikely_jump(gen_rtx_SET(pc_rtx, x));
  } else
			 ...
			 rtx value, rtx model_rtx, rtx cond)
{
  ...
  aarch64_emit_store_exclusive(mode, cond, mem, gen_lowpart(mode, new_out),
                               model_rtx);
...
aarch64_float_const_representable_p (rtx x)
{
  ...
  if (GET_MODE(x) == VOIDmode || (GET_MODE(x) == HFmode && !TARGET_FP_F16INST))
    return false;
				   ...
				   enum simd_immediate_check which)
{
  ...
  if (GET_MODE_CLASS(info.elt_mode) == MODE_FLOAT) {
    gcc_assert(info.insn == simd_immediate_info::MOV && info.u.mov.shift == 0);
    ...
    if (aarch64_float_const_zero_rtx_p(info.u.mov.value))
      ...
      if (lane_count == 1)
        ...
        snprintf(templ, sizeof(templ), "fmov\t%%0.%d%c, %s", lane_count,
  ...
  if (which == AARCH64_CHECK_MOV) {
    ...
    shift_op =
        (info.u.mov.modifier == simd_immediate_info::MSL ? "msl" : "lsl");
    if (lane_count == 1)
      snprintf(templ, sizeof(templ), "%s\t%%d0, " HOST_WIDE_INT_PRINT_HEX,
               mnemonic, UINTVAL(info.u.mov.value));
    else if (info.u.mov.shift)
      snprintf(templ, sizeof(templ),
               "%s\t%%0.%d%c, " HOST_WIDE_INT_PRINT_HEX ", %s %d", mnemonic,
               lane_count, element_char, UINTVAL(info.u.mov.value), shift_op,
               info.u.mov.shift);
    else
      snprintf(templ, sizeof(templ), "%s\t%%0.%d%c, " HOST_WIDE_INT_PRINT_HEX,
               mnemonic, lane_count, element_char, UINTVAL(info.u.mov.value));
  } else {
    /* For AARCH64_CHECK_BIC and AARCH64_CHECK_ORR.  */
    mnemonic = info.insn == simd_immediate_info::MVN ? "bic" : "orr";
    if (info.u.mov.shift)
      snprintf(templ, sizeof(templ),
               "%s\t%%0.%d%c, #" HOST_WIDE_INT_PRINT_DEC ", %s #%d", mnemonic,
               lane_count, element_char, UINTVAL(info.u.mov.value), "lsl",
               info.u.mov.shift);
    else
      snprintf(templ, sizeof(templ), "%s\t%%0.%d%c, #" HOST_WIDE_INT_PRINT_DEC,
               mnemonic, lane_count, element_char, UINTVAL(info.u.mov.value));
  }
  return templ;
}
...
aarch64_output_sve_mov_immediate (rtx const_vector)
{
  ...
  if (info.insn == simd_immediate_info::INDEX) {
    snprintf(templ, sizeof(templ),
             "index\t%%0.%c, #" HOST_WIDE_INT_PRINT_DEC
  ...
  if (GET_MODE_CLASS(info.elt_mode) == MODE_FLOAT) {
    if (aarch64_float_const_zero_rtx_p(info.u.mov.value))
      ...
      snprintf(templ, sizeof(templ), "fmov\t%%0.%c, #%s", element_char,
               float_buf);
      return templ;
    }
  }
			 ...
			 unsigned int nelt)
{
  ...
  /* The TBL instruction does not use a modulo index, so we must take care
     of that ourselves.  */
  mask = aarch64_simd_gen_const_vector_dup(vmode, one_vector_p ? nelt - 1
                                                               : 2 * nelt - 1);
  ...
  if (BYTES_BIG_ENDIAN) {
    ...
    sel = expand_simple_binop(vmode, XOR, sel, mask, NULL, 0, OPTAB_LIB_WIDEN);
  }
...
emit_unspec2 (rtx target, int code, rtx op0, rtx op1)
{
  emit_insn(gen_rtx_SET(
      target, gen_rtx_UNSPEC(GET_MODE(target), gen_rtvec(2, op0, op1), code)));
}
...
aarch64_expand_sve_vec_perm (rtx target, rtx op0, rtx op1, rtx sel)
{
  ...
  if (rtx_equal_p(op0, op1)) {
    ...
    rtx sel_mod = expand_simple_binop(sel_mode, AND, sel_reg, max_sel, NULL, 0,
                                      OPTAB_DIRECT);
  ...
  if (!CONST_VECTOR_P(sel) ||
    ...
    sel_reg = expand_simple_binop(sel_mode, AND, sel_reg, max_sel, NULL, 0,
                                  OPTAB_DIRECT);
  }
...
aarch64_evpc_trn (struct expand_vec_perm_d *d)
{
  ...
  if (!d->perm[0].is_constant(&odd) || (odd != 0 && odd != 1) ||
      !d->perm.series_p(0, 2, odd, 2) || !d->perm.series_p(1, 2, nelt + odd, 2))
    return false;
...
aarch64_evpc_ext (struct expand_vec_perm_d *d)
{
  ...
  if (d->vec_flags == VEC_SVE_PRED || !d->perm[0].is_constant(&location) ||
  ...
  emit_set_insn(d->target,
                gen_rtx_UNSPEC(d->vmode, gen_rtvec(3, d->op0, d->op1, offset),
                               UNSPEC_EXT));
  return true;
}
...
aarch64_evpc_rev_local (struct expand_vec_perm_d *d)
{
  ...
  if (d->vec_flags == VEC_SVE_PRED || !d->one_vector_p ||
      !d->perm[0].is_constant(&diff) || !diff)
    return false;
  ...
  if (d->vec_flags & VEC_SVE_DATA) {
    ...
    emit_insn(
        gen_aarch64_sve_revbhw(d->vmode, pred_mode, d->target, pred, d->op0));
    return true;
  }
...
aarch64_evpc_dup (struct expand_vec_perm_d *d)
{
  ...
  if (d->vec_flags == VEC_SVE_PRED || d->perm.encoding().encoded_nelts() != 1 ||
...
aarch64_evpc_tbl (struct expand_vec_perm_d *d)
{
  ...
  for (unsigned int i = 0; i < nelt; ++i)
    ...
    rperm[i] = GEN_INT(BYTES_BIG_ENDIAN ? d->perm[i].to_constant() ^ (nelt - 1)
...
{
  if (BYTES_BIG_ENDIAN || !d->one_vector_p || d->vec_flags != VEC_SVE_DATA ||
      d->op_vec_flags != VEC_ADVSIMD ||
      d->perm.encoding().nelts_per_pattern() != 1 ||
      !known_eq(d->perm.encoding().npatterns(), GET_MODE_NUNITS(d->op_mode)) ||
...
aarch64_evpc_sel (struct expand_vec_perm_d *d)
{
  ...
  if (d->vec_flags != VEC_SVE_DATA || unit_size > 8)
    return false;
...
aarch64_expand_sve_vec_cmp_int (rtx target, rtx_code code, rtx op0, rtx op1)
{
  ...
  rtx res =
      aarch64_sve_emit_int_cmp(target, pred_mode, code, data_mode, op0, op1);
			  ...
			  bool known_ptrue_p, rtx op0, rtx op1)
{
  ...
  rtx unspec =
      gen_rtx_UNSPEC(GET_MODE(pred), gen_rtvec(4, pred, flag, op0, op1),
				  ...
				  rtx op0, rtx op1, bool can_invert_p)
{
  ...
  case UNEQ:
    if (!flag_trapping_math) {
      ...
      op1 = force_reg(data_mode, op1);
      aarch64_emit_sve_or_fp_conds(target, UNORDERED, EQ, ptrue, true, op0,
                                   op1);
      return false;
    }
    /* fall through */
  ...
  case UNGE:
    if (flag_trapping_math) {
      ...
      op1 = force_reg(data_mode, op1);
      aarch64_emit_sve_invert_fp_cond(ordered, UNORDERED, ptrue, true, op0,
                                      op1);
      ...
      if (can_invert_p) {
        aarch64_emit_sve_fp_cond(target, code, ordered, false, op0, op1);
        return true;
      }
      aarch64_emit_sve_invert_fp_cond(target, code, ordered, false, op0, op1);
      return false;
    }
    break;
			  ...
			  rtx *ops)
{
  ...
  if (FLOAT_MODE_P(cmp_mode)) {
    if (aarch64_expand_sve_vec_cmp_float(pred, GET_CODE(ops[3]), ops[4], ops[5],
                                         true))
...
aarch64_modes_tieable_p (machine_mode mode1, machine_mode mode2)
{
  if ((aarch64_advsimd_partial_struct_mode_p(mode1) !=
      ...
      maybe_gt(GET_MODE_SIZE(mode1), 8) && maybe_gt(GET_MODE_SIZE(mode2), 8))
    return false;
  ...
  /* We specifically want to allow elements of "structure" modes to
     ...
     register.  */
  if (aarch64_vector_data_mode_p(mode1) && aarch64_vector_data_mode_p(mode2))
    return true;
...
aarch64_move_pointer (rtx pointer, poly_int64 amount)
{
  ...
  return adjust_automodify_address(pointer, GET_MODE(pointer), next, amount);
}
...
aarch64_expand_setmem (rtx *operands)
{
  ...
  if (size_p) {
    ...
    if (TARGET_MOPS && mops_cost <= libcall_cost && mops_cost <= simd_ops)
      return aarch64_expand_setmem_mops(operands);
			      ...
			      rtx label_ref)
{
  ...
  x = gen_rtx_IF_THEN_ELSE(VOIDmode, x, gen_rtx_LABEL_REF(VOIDmode, label_ref),
                           pc_rtx);
  aarch64_emit_unlikely_jump(gen_rtx_SET(pc_rtx, x));
}
  ...
  *prep_seq = get_insns();
  ...
  if (bit_code != AND) {
    ...
    if ((prev_mode == CCFPmode || prev_mode == CCFPEmode) &&
        !(prev_code == EQ || prev_code == NE || prev_code == ORDERED ||
          prev_code == UNORDERED))
...
aarch_macro_fusion_pair_p (rtx_insn *prev, rtx_insn *curr)
{
  ...
  if (simple_sets_p && aarch64_fusion_enabled_p(AARCH64_FUSE_MOV_MOVK)) {
    ...
    if (GET_CODE(set_dest) == ZERO_EXTRACT && CONST_INT_P(SET_SRC(curr_set)) &&
        CONST_INT_P(SET_SRC(prev_set)) && CONST_INT_P(XEXP(set_dest, 2)) &&
        INTVAL(XEXP(set_dest, 2)) == 16 && REG_P(XEXP(set_dest, 0)) &&
  ...
  if (simple_sets_p && aarch64_fusion_enabled_p(AARCH64_FUSE_ADRP_ADD)) {
    ...
    if (satisfies_constraint_Ush(SET_SRC(prev_set)) &&
      ...
      if (GET_CODE(SET_SRC(curr_set)) == LO_SUM &&
          ...
          rtx_equal_p(XEXP(SET_SRC(prev_set), 0), XEXP(SET_SRC(curr_set), 1)))
        return true;
    }
  }
  ...
  if (simple_sets_p && aarch64_fusion_enabled_p(AARCH64_FUSE_MOVK_MOVK)) {
    ...
    if (GET_CODE(SET_DEST(prev_set)) == ZERO_EXTRACT &&
        ...
        CONST_INT_P(SET_SRC(prev_set)) && CONST_INT_P(SET_SRC(curr_set)))
      return true;
  }
  if (simple_sets_p && aarch64_fusion_enabled_p(AARCH64_FUSE_ADRP_LDR)) {
    ...
    if (satisfies_constraint_Ush(SET_SRC(prev_set)) &&
      ...
      if (MEM_P(curr_src) && GET_CODE(XEXP(curr_src, 0)) == LO_SUM &&
          ...
          REGNO(XEXP(XEXP(curr_src, 0), 0)) == REGNO(SET_DEST(prev_set)) &&
          rtx_equal_p(XEXP(XEXP(curr_src, 0), 1), XEXP(SET_SRC(prev_set), 0)))
        return true;
    }
  }

  /* Fuse compare (CMP/CMN/TST/BICS) and conditional branch.  */
  if (aarch64_fusion_enabled_p(AARCH64_FUSE_CMP_BRANCH) && prev_set &&
      curr_set && any_condjump_p(curr) &&
  ...
  if (aarch64_fusion_enabled_p(AARCH64_FUSE_ALU_BRANCH) &&
    ...
    if (reg_referenced_p(cc_reg_1, PATTERN(curr)) && prev &&
      ...
      if (prev_type == TYPE_ALUS_SREG || prev_type == TYPE_ALUS_IMM ||
          prev_type == TYPE_LOGICS_REG || prev_type == TYPE_LOGICS_IMM)
        return true;
    }
  }

  /* Fuse ALU instructions and CBZ/CBNZ.  */
  if (prev_set && curr_set && aarch64_fusion_enabled_p(AARCH64_FUSE_ALU_CBZ) &&
      any_condjump_p(curr)) {
    /* We're trying to match:
  ...
  if (simple_sets_p &&
    ...
    if (GET_CODE(curr_src) == PLUS &&
        ...
        INTVAL(XEXP(curr_src, 1)) == polarity && REG_P(XEXP(curr_src, 0)) &&
        REG_P(SET_DEST(prev_set)) &&
...
aarch64_check_consecutive_mems (rtx *mem1, rtx *mem2, bool *reversed)
{
  ...
  if (MEM_EXPR(*mem1) && MEM_EXPR(*mem2) && MEM_OFFSET_KNOWN_P(*mem1) &&
      MEM_OFFSET_KNOWN_P(*mem2)) {
    poly_int64 expr_offset1;
    poly_int64 expr_offset2;
    tree expr_base1 =
        get_addr_base_and_unit_offset(MEM_EXPR(*mem1), &expr_offset1);
    tree expr_base2 =
        get_addr_base_and_unit_offset(MEM_EXPR(*mem2), &expr_offset2);
    if (!expr_base1 || !expr_base2 || !DECL_P(expr_base1) ||
    ...
    if (reversed) {
      if (base2) {
        rtx addr1 =
            plus_constant(Pmode, XEXP(*mem2, 0), expr_offset1 - expr_offset2);
        *mem1 = replace_equiv_address_nv(*mem1, addr1);
      } else {
        rtx addr2 =
            plus_constant(Pmode, XEXP(*mem1, 0), expr_offset2 - expr_offset1);
				...
				machine_mode mode)
{
  ...
  if (mode == SImode &&
      (aarch64_tune_params.extra_tuning_flags &
       AARCH64_EXTRA_TUNE_SLOW_UNALIGNED_LDPW) &&
      !optimize_size && MEM_ALIGN(mem_1) < 8 * BITS_PER_UNIT)
    return false;
  ...
  gcc_assert(
      known_eq(GET_MODE_SIZE(GET_MODE(mem_1)), GET_MODE_SIZE(GET_MODE(mem_2))));
...
aarch64_swap_ldrstr_operands (rtx* operands, bool load)
{
  int mem_op = load ? 1 : 0;
  bool reversed = false;
  if (!aarch64_check_consecutive_mems(operands + mem_op, operands + mem_op + 2,
                                      &reversed))
    gcc_unreachable();
...
{
  return wi::cmps(*((const HOST_WIDE_INT *)x), *((const HOST_WIDE_INT *)y));
}
				       ...
				       machine_mode mode)
{
  ...
  /* Check if the registers are of same class.  */
  rclass = REG_P(reg[0]) && FP_REGNUM_P(REGNO(reg[0])) ? FP_REGS : GENERAL_REGS;
  ...
  if (!(offvals[1] == offvals[0] + msize && offvals[3] == offvals[2] + msize))
    return false;
  ...
  if (mode == SImode &&
      (aarch64_tune_params.extra_tuning_flags &
       AARCH64_EXTRA_TUNE_SLOW_UNALIGNED_LDPW) &&
      !optimize_size && MEM_ALIGN(mem[0]) < 8 * BITS_PER_UNIT)
    return false;
			     ...
			     machine_mode mode, RTX_CODE code)
{
  ...
  gcc_assert(base != NULL_RTX && offset_1 != NULL_RTX && offset_3 != NULL_RTX);
  ...
  replace_equiv_address_nv(mem_1, plus_constant(Pmode, operands[8], new_off_1),
                           true);
  replace_equiv_address_nv(
      mem_2, plus_constant(Pmode, operands[8], new_off_1 + msize), true);
  replace_equiv_address_nv(mem_3, plus_constant(Pmode, operands[8], new_off_3),
                           true);
  replace_equiv_address_nv(
      mem_4, plus_constant(Pmode, operands[8], new_off_3 + msize), true);
...
aarch64_fpconst_pow_of_2 (rtx x)
{
  ...
  if (REAL_VALUE_NEGATIVE(*r) || REAL_VALUE_ISNAN(*r) || REAL_VALUE_ISINF(*r) ||
...
aarch64_fpconst_pow2_recip (rtx x)
{
  ...
  if (exact_real_inverse(DFmode, &r0) && !REAL_VALUE_NEGATIVE(r0)) {
...
{
  int nelts;
  if (!CONST_VECTOR_P(x) || !CONST_VECTOR_NUNITS(x).is_constant(&nelts))
    return -1;
...
{
  if (SCALAR_FLOAT_TYPE_P(t) && TYPE_MAIN_VARIANT(t) == aarch64_fp16_type_node)
...
aarch64_excess_precision (enum excess_precision_type type)
{
  ...
  case EXCESS_PRECISION_TYPE_STANDARD:
    /* We can calculate either in 16-bit range and precision or
       ...
       instructions or not.  */
    return (TARGET_FP_F16INST ? FLT_EVAL_METHOD_PROMOTE_TO_FLOAT16
			       ...
			       machine_mode to, reg_class_t)
{
  ...
  bool to_partial_advsimd_struct_p =
      (to_flags == (VEC_ADVSIMD | VEC_STRUCT | VEC_PARTIAL));
  bool from_partial_advsimd_struct_p =
      (from_flags == (VEC_ADVSIMD | VEC_STRUCT | VEC_PARTIAL));
					...
					bool explicit_p)
{
  ...
  if (maybe_ne(clonei->simdlen, 0U) &&
      ...
      (const_simdlen < 2 || const_simdlen > 1024 ||
       (const_simdlen & (const_simdlen - 1)) != 0)) {
    if (explicit_p)
      warning_at(DECL_SOURCE_LOCATION(node->decl), 0, "unsupported simdlen %wd",
                 const_simdlen);
    return 0;
  }
			   ...
			   const_tree type2)
{
  if (VECTOR_TYPE_P(type1) && VECTOR_TYPE_P(type2) &&
      !TYPE_INDIVISIBLE_P(type1) && !TYPE_INDIVISIBLE_P(type2) &&
...
{
  return mitigation_required ? (TARGET_SB ? "sb" : "dsb\tsy\n\tisb") : "";
}
...
aarch64_sls_create_blr_label (int regnum)
{
  ...
  if (optimize_function_for_size_p(cfun)) {
    ...
    if (aarch64_sls_shared_thunks[regnum] == NULL) {
      ...
      tree decl = build_decl(
          BUILTINS_LOCATION, FUNCTION_DECL, get_identifier(thunk_name),
  ...
  if (cfun->machine->call_via[regnum] == NULL)
    cfun->machine->call_via[regnum] = gen_rtx_LABEL_REF(Pmode, gen_label_rtx());
...
aarch64_parallel_select_half_p (machine_mode mode, rtx par)
{
  ...
  for (int i = 0; i < nunits; ++i) {
    ...
    if (!CONST_INT_P(elt) || !IN_RANGE(INTVAL(elt), 0, mode_nunits - 1) ||
        parset.add(elt))
      return false;
  }
  return true;
...
aarch64_optimize_mode_switching (aarch64_mode_entity entity)
{
  bool have_sme_state =
      (aarch64_cfun_incoming_pstate_za() != 0 ||
       (aarch64_cfun_has_new_state("za") && df_regs_ever_live_p(ZA_REGNUM)) ||
       (aarch64_cfun_has_new_state("zt0") && df_regs_ever_live_p(ZT0_REGNUM)));
				   ...
				   aarch64_local_sme_state prev_mode)
{
  ...
  if (mode == aarch64_local_sme_state::INACTIVE_CALLER ||
    ...
    if ((prev_mode == aarch64_local_sme_state::ACTIVE_LIVE ||
         prev_mode == aarch64_local_sme_state::ACTIVE_DEAD) &&
        mode == aarch64_local_sme_state::OFF && aarch64_cfun_has_state("zt0"))
      aarch64_save_zt0();
...
aarch64_mode_after (int entity, int mode, rtx_insn *, HARD_REG_SET live)
{
  ...
  case aarch64_mode_entity::LOCAL_SME_STATE:
    return int(aarch64_mode_after_local_sme_state(aarch64_local_sme_state(mode),
                                                  live));
  }
  gcc_unreachable();
}
...
aarch64_mode_confluence (int entity, int mode1, int mode2)
{
  ...
  case aarch64_mode_entity::LOCAL_SME_STATE:
    return int(aarch64_local_sme_confluence(aarch64_local_sme_state(mode1),
                                            aarch64_local_sme_state(mode2)));
  }
  gcc_unreachable();
}
...
aarch64_mode_backprop (int entity, int mode1, int mode2)
{
  ...
  case aarch64_mode_entity::LOCAL_SME_STATE:
    return int(aarch64_local_sme_backprop(aarch64_local_sme_state(mode1),
                                          aarch64_local_sme_state(mode2)));
  }
  gcc_unreachable();
}
		       ...
		       HARD_REG_SET &clobbered_regs, location_t loc)
{
  ...
  if (TARGET_ZA)
    for (unsigned int i = clobbers.length(); i-- > 0;) {
      rtx x = clobbers[i];
      if (REG_P(x) && (REGNO(x) == ZA_REGNUM || REGNO(x) == ZT0_REGNUM)) {
        ...
        emit_insn(REGNO(x) == ZA_REGNUM ? gen_aarch64_asm_update_za(id_rtx)
...
aarch64_switch_pstate_sm_for_landing_pad (basic_block bb)
{
  ...
  if (TARGET_STREAMING_COMPATIBLE)
    guard_label = aarch64_guard_switch_pstate_sm(IP0_REGNUM, AARCH64_FL_SM_OFF);
...
aarch64_switch_pstate_sm_for_call (rtx_call_insn *call)
{
  ...
  if (TARGET_STREAMING_COMPATIBLE)
    args_guard_label =
        aarch64_guard_switch_pstate_sm(IP0_REGNUM, callee_isa_mode);
  aarch64_sme_mode_switch_regs args_switch;
...
namespace {
  ...
  bool pass_switch_pstate_sm::gate(function * fn) {
  ...
  unsigned int pass_switch_pstate_sm::execute(function * fn) {
    basic_block bb;
    ...
    FOR_EACH_BB_FN(bb, fn) {
      ...
      auto end = BB_END(bb);
      if (JUMP_P(end) && find_reg_note(end, REG_NON_LOCAL_GOTO, NULL_RTX) &&
...
namespace selftest {
  ...
  static void aarch64_test_loading_full_dump() {
  ...
  static void aarch64_test_fractional_cost() {
  ...
  static void aarch64_run_selftests(void) {
...
#define TARGET_SHRINK_WRAP_COMPONENTS_FOR_BB aarch64_components_for_bb

#undef TARGET_SHRINK_WRAP_DISQUALIFY_COMPONENTS
#define TARGET_SHRINK_WRAP_DISQUALIFY_COMPONENTS aarch64_disqualify_components
...
#define TARGET_SHRINK_WRAP_SET_HANDLED_COMPONENTS aarch64_set_handled_components
...
#define TARGET_VECTOR_MODE_SUPPORTED_ANY_TARGET_P                              \
  aarch64_vector_mode_supported_any_target_p
...
#define TARGET_ATOMIC_ASSIGN_EXPAND_FENV aarch64_atomic_assign_expand_fenv
...
#define TARGET_VECTORIZE_VEC_PERM_CONST aarch64_vectorize_vec_perm_const
...
#define TARGET_VECTORIZE_EMPTY_MASK_IS_EXPENSIVE aarch64_empty_mask_is_expensive
#undef TARGET_PREFERRED_ELSE_VALUE
#define TARGET_PREFERRED_ELSE_VALUE aarch64_preferred_else_value
